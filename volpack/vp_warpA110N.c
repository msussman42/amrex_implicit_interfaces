...
void
VPWarpA110N (in_image, in_width, in_height, in_bytes_per_scan,
	  out_image, out_width, out_height, out_bytes_per_scan,
	  warp_matrix)
GrayIntPixel *in_image;		/* input image data */
int in_width;			/* size of input image */
int in_height;
int in_bytes_per_scan;		/* bytes per scanline in input image */
unsigned char *out_image;		/* output image data */
				...
				/* [   1  ]                 [  1  ] */
{
    ...
    unsigned char *out_ptr;		/* pointer to current pixel of output image */
    ...
    GrayIntPixel *in_ptr;	/* pointer to current pixel of input image */
	...
	float gray_acc; int gray_acc_int;
			/* pixel accumulator */
    double denom;
    int c;
	...
	while (scans_to_next_vertex > 0) {
	    ...
	    in_ptr = (GrayIntPixel *)(((char *)in_image + in_y_int *
	    ...
	    if (lft_zero_cnt > 0) {
		bzero(out_ptr, lft_zero_cnt * ((1) + (0)));
		out_ptr += lft_zero_cnt * ((1) + (0));
	    }
	...
	gray_acc += (wptr[0]) * (in_ptr[0].clrflt);
	;
		    }
		    if (in_y_int+1 >= 0 && in_y_int+1 < in_height) {
			
	gray_acc += (wptr[2]) * (in_ptr[in_width].clrflt);
	;
		    }
		}
	...
	gray_acc += (wptr[1]) * (in_ptr[1].clrflt);
	;
		    }
		    if (in_y_int+1 >= 0 && in_y_int+1 < in_height) {
			
	gray_acc += (wptr[3]) * (in_ptr[in_width + 1].clrflt);
	;
		    }
		}
	...
	if (gray_acc_int > 255)
	    gray_acc_int = 255;
	((out_ptr)[0]) = gray_acc_int;
	    ;
		out_ptr += ((1) + (0));
		xfrac += xfrac_incr;
		yfrac += yfrac_incr;
		if ( (unsigned int)xfrac & ~0x7FFFFFFF ) {
		    xfrac &= 0x7FFFFFFF;
		...
		if ( (unsigned int)yfrac & ~0x7FFFFFFF ) {
		    yfrac &= 0x7FFFFFFF;
		...
		ASSERT((GrayIntPixel *)(((char *)in_image + in_y_int *
	...
	gray_acc = (wptr[0]) * (in_ptr[0].clrflt) +
		   (wptr[2]) * (in_ptr[in_width].clrflt) +	
		   (wptr[1]) * (in_ptr[1].clrflt) +
		   (wptr[3]) * (in_ptr[in_width+1].clrflt);
	;
	...
	((out_ptr)[0]) = gray_acc_int;
	    ...
	    if (rgt_zero_cnt > 0) {
		bzero(out_ptr, rgt_zero_cnt * ((1) + (0)));
		out_ptr += rgt_zero_cnt * ((1) + (0));
	    }

	    /* go on to next scan */
	    scans_to_next_vertex--;
	    out_scan_y++;
	    out_ptr += out_bytes_per_scan - out_width * ((1) + (0));
