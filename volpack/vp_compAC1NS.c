...
void
VPCompAC1NS (vpc, icount, jcount, k, slice_depth_cueing_dbl, intimage,
...
#define SET_VOXELS_LOADED	voxels_loaded = 1
    ...
    shade_index=num_materials*1*ShortField(topRLEdata - voxel_istride,norm_offset);
    weight_index = num_materials * ByteField(topRLEdata - voxel_istride, wgt_offset);
    
        
	
    top_clr = 
    shade_table[shade_index + 1*0 + 0] * 
	((num_materials > 1) ? weight_table[weight_index] : (float)1.0);
	
        top_sclr = 
    shadow_table[shade_index + 1*0 + 0] * 
	((num_materials > 1) ? weight_table[weight_index] : (float)1.0);
        for (m = 1; m < num_materials; m++) {
	     
	
    top_clr += 
    shade_table[shade_index + 1*m + 0] * 
	weight_table[weight_index + m];
	
        top_sclr += 
    shadow_table[shade_index + 1*m + 0] * 
	weight_table[weight_index + m];
	};
...
#           endif
    ...
    shade_index=num_materials*1*ShortField(botRLEdata - voxel_istride,norm_offset);
...
#           endif
    ...
    shade_index=num_materials*1*ShortField(topRLEdata,norm_offset);
...
#           endif
    ...
    shade_index=num_materials*1*ShortField(botRLEdata,norm_offset);
