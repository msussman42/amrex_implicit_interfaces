...
static void ResampleFloat ANSI_ARGS((vpContext *vpc, int num_dimens,
    int *src_dimens, int *dst_dimens, int *src_strides, int *dst_strides,
    float *in_array, float *out_array, FilterTemplate *tmplate));
...
void *out_array;	/* storage for output array */
{
    ...
    FilterTemplate *tmplate;	/* filter template */
    ...
    Alloc(vpc, tmplate, FilterTemplate *, out_x_count*sizeof(FilterTemplate),
	  "FilterTemplate");
	...
	tmplate[out_x].in_ptr_offset = in_offset + tap_min * in_x_stride;
	tmplate[out_x].wptr = &weights[(phase >> unused_phase_bits) * num_taps
					+ tap_min];
	tmplate[out_x].tap_min = tap_min;
	tmplate[out_x].tap_max = tap_max;
	phase += phase_incr;
	if ( (unsigned int) phase & ~0x7FFFFFFF ) {
	    phase &= 0x7FFFFFFF;
	    index += index_incr + 1;
	...
	ResampleUchar(vpc, num_dimens, src_dimens, dst_dimens, src_strides,
		      dst_strides, (unsigned char*)in_array, (unsigned char*)out_array, tmplate);
	break;
    case VP_USHORT:
	ResampleUshort(vpc, num_dimens, src_dimens, dst_dimens, src_strides,
		       dst_strides, (unsigned short*)in_array, (unsigned short*)out_array, tmplate);
	break;
    case VP_FLOAT:
	ResampleFloat(vpc, num_dimens, src_dimens, dst_dimens, src_strides,
		      dst_strides, (float*)in_array, (float*)out_array, tmplate);
	break;
    default:
	Dealloc(vpc, tmplate);
	return(VPSetError(vpc, VPERROR_BAD_VALUE));
...
static void
ResampleUchar(vpc, num_dimens, src_dimens, dst_dimens, src_strides,
	      dst_strides, in_array, out_array, tmplate)
vpContext *vpc;
int num_dimens;		/* number of dimensions in the two arrays */
int *src_dimens;	/* sizes of source array dimensions */
int *dst_dimens;	/* sizes of destination array dimensions (must
...
int filter_type;	/* type of filter (VP_BOX_FILTER, etc.) */
{
	...
	break;
    default:
	return(VPSetError(0, VPERROR_BAD_OPTION));
    }
    vpc = vpCreateContext();
