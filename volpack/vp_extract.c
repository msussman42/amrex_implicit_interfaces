...
int dst_zstride;
{
	...
	if (field == VP_OPACITY_FIELD)
	    return(ClassifyRawVolume(vpc, 0, x0, y0, z0, x1, y1, z1, (unsigned char*)dst,
				     dst_xstride, dst_ystride, dst_zstride));
	else if (field == VP_CORRECTED_OPAC_FIELD)
	    return(ClassifyRawVolume(vpc, 1, x0, y0, z0, x1, y1, z1, (unsigned char*)dst,
				     dst_xstride, dst_ystride, dst_zstride));
	else if (field == VP_COLOR_FIELD)
	    return(ShadeRawVolume(vpc, x0, y0, z0, x1, y1, z1, (unsigned char*)dst,
				  dst_xstride, dst_ystride, dst_zstride));
	else
...
int dst_zstride;
{
    ...
    ystride = vpc->ystride;
    zstride = vpc->zstride;
    voxel = (unsigned char*)vpc->raw_voxels;
	...
	for (y = y0; y <= y1; y++) {
		...
		if (field_size == 1)
		    ...
		    ShortFieldSet(dstptr, 0, ShortField(voxel, field_offset));
		else
		    IntFieldSet(dstptr, 0, IntField(voxel, field_offset));
		dstptr += dst_xstride;
		voxel += xstride;
	    }
	...
	for (j = 0; j < jlen; j++) {
		...
		if (i >= i0 && i <= i1 && j >= j0 && j <= j1) {
			...
			if (field_size == 1)
			    ...
			    ShortFieldSet(dstptr, 0, 0);
			else
			    IntFieldSet(dstptr, 0, 0);
		    }
		    dstptr += dst_istride;
		} else {
		    if (is_non_zero)
