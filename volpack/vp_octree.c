...
void *mm_pyramid[VP_MAX_OCTREE_LEVELS];
{
    ...
    pyr_dst = (char*)mm_pyramid[pyr_levels-1];
	...
	for (y = 0; y < nodes_per_side; y++) {
		...
		for (p = 0; p < num_params; p++) {
			...
			ShortFieldSet(pyr_dst, node_offset[p], min_value[p]);
			ShortFieldSet(pyr_dst, node_offset[p]+2, max_value[p]);
		    }
		}
	...
	Debug((vpc, VPDEBUG_PYRAMID, "Pyramid Level %d:\n", level));
	pyr_dst = (char*)mm_pyramid[level];
	pyr_node = (char*)mm_pyramid[level+1];
	pyr_offsets[0] = 0;
...
int *octree_offset;	/* bytes from root of octree to next free location */
{
	...
	if (subdivide) {
		...
		IntFieldSet(parent_node, child_offset, *octree_offset);
		Debug((vpc, VPDEBUG_OCTREE,
	    ...
	    *octree_offset += 8 * child_bytes_per_node;
	    ...
	    if (parent_node != NULL) {
		IntFieldSet(parent_node, child_offset, 0);
	    }
	}
    }
}
...
vpContext *vpc;
{
    ...
    octree_root = (char*)vpc->mm_octree->root;
...
int icount;		/* scanline length */
{
	...
	node = (char*)level_stack[level].child2;
...
int max_level;
{
    ...
    if ((retcode = VPCheckClassifier(vpc)) == 0)
	return(retcode);
