...
#if defined(DUMP_SHADOW_VOLUME) || defined(DUMP_GRAY_VOLUME)
#ifdef WIN32
#include <io.h>
#else
#include <unistd.h>
...
void (*composite_func)(vpContext*, ...); /* function to do the compositing */
{
	...
	if (color_channels == 1) {
	    VPWarpA101N(vpc->int_image.gray_intim, vpc->intermediate_width,
			...
			(unsigned char*)vpc->image, vpc->image_width, vpc->image_height,
    ...
    fflush(stdout);
    if ((dump_fd = open("shadow.dump", O_BINARY|O_CREAT|O_TRUNC|O_WRONLY, 0644)) < 0)
	VPBug("open failed");
    ...
    fflush(stdout);
    if ((dump_fd = open("gray.dump", O_BINARY|O_CREAT|O_TRUNC|O_WRONLY, 0644)) < 0)
	VPBug("open failed");
