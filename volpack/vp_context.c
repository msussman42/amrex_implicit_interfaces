...
vpResult
vpWindow(vpc, type, left, right, bottom, top, nnear, ffar)
vpContext *vpc;
int type;
double left, right, bottom, top, nnear, ffar;
{
    ...
    if (left >= right || bottom >= top || nnear >= ffar)
	return(VPSetError(vpc, VPERROR_BAD_VALUE));
	...
	projectm[2][2] = 2. / (ffar - nnear);
	projectm[0][3] = (left + right) / (left - right);
	projectm[1][3] = (bottom + top) / (bottom - top);
	projectm[2][3] = (nnear + ffar) / (nnear - ffar);
    } else {
	/* XXX perspective rendering not available yet */
	return(VPSetError(vpc, VPERROR_BAD_OPTION));
#ifdef notdef
	projectm[0][0] = 2. * nnear / (right - left);
	projectm[1][1] = 2. * nnear / (top - bottom);
	projectm[2][2] = (nnear + ffar) / (nnear - ffar);
	projectm[0][2] = (right + left) / (right - left);
	projectm[1][2] = (top + bottom) / (top - bottom);
	projectm[3][2] = -1.;
	projectm[2][3] = 2. * ffar * nnear / (nnear - ffar);
#endif
    }
    if (vpc->concat_left)
...
void (*func)();
{
	...
	vpc->log_alloc_func = (void (*)(void*, int, char*, int, char*, void*))func;
	break;
    case VP_LOG_FREE_FUNC:
	vpc->log_free_func = (void(*)(void*, void*))func;
	break;
    case VP_STATUS_FUNC:
	vpc->status_func = (void(*)(double, void*))func;
	break;
    case VP_READ_FUNC:
	if (func == NULL)
	    vpc->read_func = read;
...
#ifdef WIN32
	    vpc->read_func = (int (*)(int, void*, size_t))func;
#else
	    vpc->read_func = (ssize_t (*)(int, void*, size_t))func;
#endif
	break;
    case VP_WRITE_FUNC:
	if (func == NULL)
...
#ifdef WIN32
	    vpc->write_func = (int(*)(int, const void*, size_t))func;
#else
	    vpc->write_func = (ssize_t (*)(int, const void*, size_t))func;
#endif
	break;
    case VP_MMAP_FUNC:
	vpc->mmap_func = (void*(*)(int, unsigned int, void*))func;
	break;
	       ...
	       )
{
  switch (option)
    {
    case VP_SHADE_FUNC:
      *pptr = (void(*)()) vpc->shade_func;
      break;
    case VP_LOG_ALLOC_FUNC:
      *pptr = (void(*)()) vpc->log_alloc_func;
      break;
    case VP_LOG_FREE_FUNC:
      *pptr = (void(*)()) vpc->log_free_func;
      break;
    case VP_STATUS_FUNC:
      *pptr = (void(*)()) vpc->status_func;
      break;
    case VP_READ_FUNC:
      *pptr = (void(*)()) vpc->read_func;
      break;
    case VP_WRITE_FUNC:
      *pptr = (void(*)()) vpc->write_func;
      break;
    case VP_MMAP_FUNC:
      *pptr = (void(*)()) vpc->mmap_func;
      break;
    default:
      return(VPSetError(vpc, VPERROR_BAD_OPTION));
    }
