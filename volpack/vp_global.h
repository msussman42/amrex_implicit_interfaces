...
#define rint(x)  ((int)(( (x) < 0.0) ? ceil( (x)-0.5) : floor((x)+0.5)))
...
#   define GET_TIME(vpc, t)	gettimeofday(&t, 0)
...
#ifdef BL_BYTEORDER_LITTLE_ENDIAN
#define VP_BYTE0_OFFSET  0
#define VP_BYTE8_OFFSET  1
#define VP_BYTE16_OFFSET 2
#define VP_BYTE24_OFFSET 3
#else
#define VP_BYTE0_OFFSET  3
#define VP_BYTE8_OFFSET  2
#define VP_BYTE16_OFFSET 1
#define VP_BYTE24_OFFSET 0
#endif
...
#define ShortField(voxel, offset)					\
     (									\
      (*((unsigned char *) (voxel) + (offset) + (VP_BYTE8_OFFSET)) << 8) |		\
      (*((unsigned char *) (voxel) + (offset) + (VP_BYTE0_OFFSET)) << 0)		\
      )
#define ShortFieldSet(voxel, offset, value)				\
do									\
{									\
  *((unsigned char *) (voxel) + (offset) + (VP_BYTE8_OFFSET)) = (unsigned char) (((value) & 0xff00) >> 8); \
  *((unsigned char *) (voxel) + (offset) + (VP_BYTE0_OFFSET)) = (unsigned char) (((value) & 0x00ff) >> 0); \
}									\
while(0)

#define IntField(voxel, offset) \
     (									\
      (*((unsigned char *) (voxel) + (offset) + (VP_BYTE24_OFFSET)) << 24) |		\
      (*((unsigned char *) (voxel) + (offset) + (VP_BYTE16_OFFSET)) << 16) |		\
      (*((unsigned char *) (voxel) + (offset) + (VP_BYTE8_OFFSET)) << 8) |		\
      (*((unsigned char *) (voxel) + (offset) + (VP_BYTE0_OFFSET)) << 0)		\
      )
#define IntFieldSet(voxel, offset, value)				\
do									\
{									\
  *((unsigned char *) (voxel) + (offset) + (VP_BYTE24_OFFSET)) = (unsigned char) (((value) & 0xff000000) >> 24);	\
  *((unsigned char *) (voxel) + (offset) + (VP_BYTE16_OFFSET)) = (unsigned char) (((value) & 0x00ff0000) >> 16);	\
  *((unsigned char *) (voxel) + (offset) + (VP_BYTE8_OFFSET)) = (unsigned char) (((value) & 0x0000ff00) >> 8); \
  *((unsigned char *) (voxel) + (offset) + (VP_BYTE0_OFFSET)) = (unsigned char) (((value) & 0x000000ff) >> 0); \
}									\
while(0)
...
struct _vp_context {			/* vpContext */
    ...
    /* callback functions */
    void (*shade_func) ANSI_ARGS((void*, ...));		/* shade a voxel */
#ifndef WIN32
    ssize_t (*write_func) ANSI_ARGS((int, const void *, size_t)); /* write to file */
    ssize_t (*read_func) ANSI_ARGS((int, void *, size_t));  /* read from file */
#else
    int (*write_func) ANSI_ARGS((int, const void*, size_t));
    int (*read_func) ANSI_ARGS((int, void*, size_t));
#endif
...
extern void		    VPRenderAffine ANSI_ARGS((vpContext *vpc,
				int algorithm, void (*composite_func)(vpContext*,...)));
