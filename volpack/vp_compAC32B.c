...
void
VPCompAC32B (vpc, icount, jcount, k, slice_depth_cueing_dbl, intimage,
...
#define SET_VOXELS_LOADED	voxels_loaded = 1
...
#ifdef RLEVOLUME
    topRLEdata = (char*)voxel_data;
    botRLEdata = (char*)voxel_data;
...
#ifdef CALLBACK
#ifdef GRAYSCALE
    shade_func = (void(*)(void*, float*, void*))vpc->shade_func;
#endif
#ifdef RGB
    shade_func = (void(*)(void*, float*, float*, float*, void*))vpc->shade_func;
#endif
    ...
    shade_index=2*3*ShortField(topRLEdata - voxel_istride,norm_offset);
    weight_index = 2 * ByteField(topRLEdata - voxel_istride, wgt_offset);
    
	
	
    top_rclr = 
    shade_table[shade_index + 3*0 + 0] * weight_table[weight_index + 0];
	
    top_gclr = 
    shade_table[shade_index + 3*0 + 1] * weight_table[weight_index + 0];
	
    top_bclr = 
    shade_table[shade_index + 3*0 + 2] * weight_table[weight_index + 0];
	;
	;
	;
    ...
    top_rclr += 
    shade_table[shade_index + 3*1 + 0] * weight_table[weight_index + 1];
	
    top_gclr += 
    shade_table[shade_index + 3*1 + 1] * weight_table[weight_index + 1];
	
    top_bclr += 
    shade_table[shade_index + 3*1 + 2] * weight_table[weight_index + 1];
	;
	;
	;
...
#           endif
    ...
    shade_index=2*3*ShortField(botRLEdata - voxel_istride,norm_offset);
    weight_index = 2 * ByteField(botRLEdata - voxel_istride, wgt_offset);
...
#           endif
    ...
    shade_index=2*3*ShortField(topRLEdata,norm_offset);
    weight_index = 2 * ByteField(topRLEdata, wgt_offset);
...
#           endif
    ...
    shade_index=2*3*ShortField(botRLEdata,norm_offset);
    weight_index = 2 * ByteField(botRLEdata, wgt_offset);
