MODULE MAT_Greens15
  ...
  use CubicSpline4,  only : CurveFit, NewCoords ! subroutine, variable
...
! program variables
  ! fixed variables
  integer		:: M=10, N=1, & ! # pts / panel; # panels / shortest side
	CubicSplineFit=1  			! if '0', linear spline fit. if '1', cubic spline fit.
  ...
  print *,"in matgrn llo,lhi,glo,ghi,dx,dy,domlo,domhi,size ",llo1,llo2, lhi1, lhi2, gridlo1, gridlo2,gridhi1, gridhi2, dx, dy, domlo1, domlo2, domhi1, domhi2,sizeLevel
                                                                                
  do imark=1,sizeLevel
   print *,"i,x,y ",imark,xarray(imark),yarray(imark)
  enddo
  ...
  write(*,fmt=10), 'Done sorting after ', timeCount
  ...
  call cpu_time(t2); timeCount = t2-t1
  write(*, fmt=12) 'Panels defined after t= ', timeCount
  ...
  call cpu_time(t2); timeCount = t2-t1
  write(*, fmt=12) 'Greens function done after t= ', timeCount
  ...
  write(*, fmt=12) 'Inverting takes t= ', timeCount
  ...
  do imark=1,sizeLevel
   print *,"i,x,y,E ",imark,xarray(imark),yarray(imark),Efield(imark)
  enddo
...
! ERROR listings
990 print *, 'Error open Sigmavar'
991 print *, 'Error read Sigmavar'
992 print *, 'Error write Sigmavar'
993 print *, 'Error close Sigmavar'

999 print *, 'Done with subroutine <MAT_Greens>'

end SUBROUTINE MAT_Greens

end MODULE MAT_Greens15
