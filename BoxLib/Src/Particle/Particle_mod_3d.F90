module BoxLib_particle_module
  ...
  public :: BoxLib_particle_set_position, BoxLib_particle_get_position, &
       BoxLib_deposit_cic, BoxLib_interpolate_cic

contains

  subroutine BoxLib_particle_set_position (particles, ns, np, x, y, z) &
       bind(c,name='BoxLib_particle_set_position')
    integer(c_int)  , intent(in   ), value :: ns, np
    real(c_real), intent(inout)        :: particles(ns,np)
    real(c_real), intent(in   )        :: x(np), y(np), z(np)
  ...
  subroutine BoxLib_particle_get_position (particles, ns, np, x, y, z) &
       bind(c,name='BoxLib_particle_get_position')
    integer(c_int)  , intent(in   ), value :: ns, np
    real(c_real), intent(in   )        :: particles(ns,np)
    real(c_real), intent(  out)        :: x(np), y(np), z(np)
  ...
  subroutine BoxLib_deposit_cic(particles, ns, np, rho, lo, hi, plo, dx) &
       bind(c,name='BoxLib_deposit_cic')
    integer, value       :: ns, np
    real(c_real)     :: particles(ns,np)
    integer              :: lo(3)
    integer              :: hi(3)
    real(c_real)     :: rho(lo(1):hi(1), lo(2):hi(2), lo(3):hi(3))
    real(c_real)     :: plo(3)
    real(c_real)     :: dx(3)

    integer i, j, k, n
    real(c_real) wx_lo, wy_lo, wz_lo, wx_hi, wy_hi, wz_hi
  ...
  end subroutine BoxLib_deposit_cic

  subroutine BoxLib_interpolate_cic(particles, ns, np, acc, lo, hi, ncomp, plo, dx) &
       bind(c,name='BoxLib_interpolate_cic')
    integer, value       :: ns, np, ncomp
    real(c_real)     :: particles(ns,np)
    integer              :: lo(3)
    integer              :: hi(3)
    real(c_real)     :: acc(lo(1):hi(1), lo(2):hi(2), lo(3):hi(3), ncomp)
    real(c_real)     :: plo(3)
    real(c_real)     :: dx(3)
    real(c_real)     :: acceleration(ncomp)
  ...
  end subroutine BoxLib_interpolate_cic
