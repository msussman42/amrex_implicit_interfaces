...
BoxArray&
BoxArray::growcoarsen (int n, const int iv) {
...
BoxArray::complement (const Box& bx) const
{
 ...
 if (!empty()) {
  ...
  for (IntVect iv = cbx.smallEnd(), End = cbx.bigEnd(); 
   ...
   if (it != TheEnd) {
    for (std::vector<int>::const_iterator v_it = it->second.begin(), 
         v_end = it->second.end(); 
     ...
     if (isect.ok()) {
      for (BoxList::iterator bli = bl.begin(); bli != bl.end(); ) {
       // *bli intersect with the compliment of isect. 
...
BoxArray
BoxLib::convert (const BoxArray& ba, const IntVect& typ)
{
    BoxArray ba2 = ba;
