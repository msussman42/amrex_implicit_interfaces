...
IntVect FabArrayBase::mfiter_tile_size(D_DECL(1024000, 8, 8));
...
{ 
	...
	bx &= m_dstdomain;
         ...
         // the boxes outside of srcba but intersect with bx.
         // "leftover" boxes needed to be filled with interpolated
         // information.
			 ...
			 const BoxConverter& coarsener)
{
    ...
    m_TheFillPatchCache.insert(er_it.second, 
      FPinfoCache::value_type(dstkey,new_fpc));
...
FabArrayBase::buildTileArray (const IntVect& tileSize, TileArray& ta) const
{
	...
	for (std::vector<int>::const_iterator it = local_idxs.begin(); it != local_idxs.end(); ++it)
	{
	    ...
	    const Box& bx = boxarray.getCellCenteredBox(K);
            ...
            //  ParticleContainer::getTileIndex function!!!
            //
...
MFIter::Initialize ()
{
 if (flags & SkipInit) {
  ...
  endIndex     = index_map->size();
   ...
   // if tile_size=(0,0,0), then pta points to a list of grids stored 
   // on the present processor.
 ...
 if (! typ.cellCentered()) {
  ...
  for (int d=0; d<BL_SPACEDIM; ++d) {
   if (typ.nodeCentered(d)) { // validbox should also be nodal in d-direction.
    if (bx.bigEnd(d) < Big[d]) { // not a tile next to the validbox boundary.
     bx.growHi(d,-1);
    }
   }
  }
...
MFIter::tilebox (const IntVect& nodal) const
{
 ...
 if (! new_typ.cellCentered()) {
  ...
  for (int d=0; d<BL_SPACEDIM; ++d) {
   if (new_typ.nodeCentered(d)) { // validbox should also be nodal in d-dir.
 ...
 return bx;
} // subroutine tilebox (const IntVect& nodal)
...
{ 
    ...
    for (int d=d0; d<=d1; ++d) {
     // validbox should also be cell-centered in d-direction.
...
Box 
MFIter::growntileboxTENSOR (int datatype,int ng,int dir) const 
{
 Box bx = tilebox();
 const Box& vbx = validbox();
