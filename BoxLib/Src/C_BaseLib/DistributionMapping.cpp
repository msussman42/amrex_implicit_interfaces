...

{
    ...
    for (int i = 0; i < nprocs; ++i)
    {
        ...
        if ((totalvol/(i+1)) > volpercpu &&  // Too much for this bin.
            loop_cnt > 1                 &&  // More than one box in this bin.
    ...
    for (int i = 0; i < nprocs; ++i)
        local_cnt += v[i].size();
    BL_ASSERT(local_cnt == tokens.size());
#endif
}
...
DistributionMapping::TranslateProcMap(const Array<int> &pm_old, const MPI_Group group_new, const MPI_Group group_old)
{
 ...
 const int* pm_old_data=pm_old.dataPtr();

 BL_MPI_REQUIRE( 
  MPI_Group_translate_ranks(group_old, pm_old.size(), 
   const_cast<int*>(pm_old_data), 
   group_new, pm_new.dataPtr()) );
