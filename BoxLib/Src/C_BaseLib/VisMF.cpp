              ...
              bool               set_ghost)
{
    ...
    if(FArrayBox::getFormat() == FABio::FAB_NATIVE) {
     ...
     BoxLib::Error("FArrayBox::getFormat() not supported");
    }
	     ...
	     int coordinatorProc)
{
    ...
    if (mf.empty()) {
     ...
     mf.define(hdr.m_ba,hdr.m_ncomp,hdr.m_ngrow,dm,Fab_allocate);
    } else {
     if (! BoxLib::match(hdr.m_ba,mf.boxArray()))
      BoxLib::Error("hdr.m_ba invalid");
    }
