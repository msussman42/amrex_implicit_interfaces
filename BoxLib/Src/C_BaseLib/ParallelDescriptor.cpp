     ...
     const Array<Array<int> > &sidecarRanksInAll, bool printRanks)
{
    ...
    if(nSidecars > 0) {
      ...
      const int* compRanks_data=compRanksInAll.dataPtr();

      BL_MPI_REQUIRE( 
       MPI_Group_incl(m_group_all, compRanksInAll.size(), 
        const_cast<int*>(compRanks_data), 
        &m_group_comp) );
	...
	if(sidecarRanksInAll[i].size() > 0) {
          ...
          const int* sidecarRanks_data=sidecarRanksInAll[i].dataPtr();

          BL_MPI_REQUIRE( 
           MPI_Group_incl(m_group_all, sidecarRanksInAll[i].size(), 
             const_cast<int*> (sidecarRanks_data),
	     &m_group_sidecar[i]) );
