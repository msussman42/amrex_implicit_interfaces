...
void advance_LS (MultiFab& phi_old,
    ...
    for (MFIter mfi(phi); mfi.isValid(); ++mfi){
        ...
        init_vel(BL_TO_FORTRAN_BOX(bx),
                     ...
                     dx, geom.ProbLo(), geom.ProbHi(), &time);
    }
...
void redist (MultiFab& phi_old,
    ...
    int redist_method = 2; //1: pde based (Sussman, Smereka, Osher), 2: crossing time (Sethian)
    const Real* dx = geom.CellSize();
    ...
    while (iter_redist <= max_iter){//4){
        ...
        for (MFIter mfi(phi); mfi.isValid(); ++mfi){
            ...
            if (redist_method == 1 || iter_redist>42){
        ...
        if (1==0){  
            const std::string& pltfile = amrex::Concatenate("dist",iter_redist,5);
            WriteSingleLevelPlotfile(pltfile, dist, {"dist"}, geom, time, iter_redist);
        }
