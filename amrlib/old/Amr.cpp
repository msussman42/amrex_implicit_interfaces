...
int Amr::nCycle (int level) const {
    return n_cycle[level];
}
int Amr::nCycleSave (int level) const {
    return n_cycle_save[level];
}

int
Amr::bFactor(int level) const
{
    return blocking_factor[level];
}
...
MultiFab*
Amr::derive (const std::string& name,int is_old,
             int lev,int ngrow)
{
    return amr_level[lev].derive(name,is_old,ngrow);
}
    ...
    amr_level(PArrayManage)
{
    ...
    //
    levelbld = getLevelBld(); // defined in NSBld.cpp
    ...
    if (pp.contains("nosub"))
     BoxLib::Error("nosub no longer used, set amr.n_cycle=1 1...");
    ...
    level_cells.resize(nlev);
    regrid_int.resize(nlev);
    n_cycle.resize(nlev);
    n_cycle_save.resize(nlev);
    ...
    for (i = 0; i < nlev; i++)
    {
        dt_level[i]    = 1.0; // Something nonzero so old & new will differ
        level_steps[i] = 0;
        level_count[i] = 0;
        level_cells[i] = 0;
        regrid_int[i]  = 0;
    ...
    int got_ncycle = pp.queryarr("n_cycle",n_cycle,0,nlev);
    int got_ncycle_save = pp.queryarr("n_cycle",n_cycle_save,0,nlev);
    if (got_ncycle==1) {
     if (n_cycle[0]!=1)
      BoxLib::Error("set amr.n_cycle[0]=1!");
     sub_cycle = false;
     for (i=0;i<nlev;i++) {
      if (n_cycle[i]!=1)
       sub_cycle = true;
      if (n_cycle[i]<=0)
       BoxLib::Error("amr.n_cycle <=0!");
     }
    } else
     BoxLib::Error("define amr.n_cycle (amr.nosub no longer used)");
...
void
Amr::set_ncycle_finest(int new_ncycle_finest) {

 n_cycle[finest_level]=new_ncycle_finest;
}
           ...
           Real stop_time)
{
    ...
    {
        is_first_step_after_restart=1;
        restart(restart_file);
    }
    else
    {
        is_first_step_after_restart=0;
                  ...
                  Real stop_time)
{
    ...
    cumtime = strt_time;
    //
    // Define base level grids.  Define data at level 0.
    //
    defBaseLevel(strt_time);  // amr_level[0].initData();
    //
    // Compute dt and set time levels of all grid data.
    //
    amr_level[0].computeInitialDt(dt_level,stop_time);

    if (max_level > 0)
        bldFineLevels(strt_time);  // amr_level[new_finest].initData();

      // newtime=strt_time, oldtime=strt_time-dt_level
    for (int lev = 0; lev <= finest_level; lev++)
        amr_level[lev].setTimeLevel(strt_time,dt_level[lev]);

    //
    // Perform any special post_initialization operations. lev=0
    //
    amr_level[0].post_init(stop_time);
...
Amr::restart (const std::string& filename)
{
    ...
    for (i = 0; i <= mx_lev; i++) is >> level_cells[i];
...
Amr::checkPoint ()
{
    ...
    if (verbose && ParallelDescriptor::IOProcessor())
        std::cout << "CHECKPOINT: check_file_root= " << check_file_root << ' ' <<
        "level_steps(0)= " << level_steps[0] << std::endl;
    ...
    if (ParallelDescriptor::IOProcessor())
    {
        ...
        HeaderFile << '\n';
        for (i = 0; i <= max_level; i++) HeaderFile << level_cells[i] << ' ';
        HeaderFile << '\n';
    ...
    if (ParallelDescriptor::IOProcessor())
        std::cout << "checkPoint() time = "
                  ...
                  << " secs." << std::endl;
    ParallelDescriptor::Barrier();
    std::cout << "after checkpoint on processor " <<
               ...
               Real prev_dt,Real stop_time)
{
	...
	writePlotFile(plot_file_root,level_steps[0]);
    } else if ((level==0)&&(level_steps[0]==0)&&(plotfile_on_restart))
	writePlotFile(plot_file_root,level_steps[0]);
    ...
    Real dt_new=amr_level[level].advance(time,dt_level[level],
     iteration,niter,sub_cycle);

    level_steps[level]++;
    level_count[level]++;
    level_cells[level]+=amr_level[level].countCells();
    ...
    if (verbose && ParallelDescriptor::IOProcessor())
    {
        std::cout << "Advanced "
                  ...
                  << level
                  << std::endl;
        std::cout << "Advanced a total number of "
                  << level_cells[level]
                  << " cells at level "
                  << level
                  << " after "
    ...
    if (level < finest_level) {
     ...
     Real prev_fine_dt=prev_dt/ncycle;

     for (int i = 1; i <= ncycle; i++)
      timeStep(lev_fine,time+(i-1)*dt_level[lev_fine],i,ncycle,
          prev_fine_dt,stop_time);
    }

    amr_level[level].post_timestep(iteration,sub_cycle,stop_time);
}

void 
Amr::output_checks(Real cumtimeParam,int& check_test,int& plot_test,
  long& levelstepszero,int endofstep) {

  levelstepszero=level_steps[0];
  if (endofstep==0)
   levelstepszero++;

  check_test = 0;
  if (check_per > 0.0) {
   const int num_per = int((cumtimeParam+.001*dt_level[0]) / check_per);
   const Real resid  = cumtimeParam - num_per * check_per;

   if (resid < .001*dt_level[0]) check_test = 1;
  }
  if (check_test==0) {
   if ((check_int > 0)&&(levelstepszero % check_int == 0))
    check_test=1;
  }

  plot_test = 0;
  if (plot_per > 0.0) {
   const int num_per = int((cumtimeParam+.001*dt_level[0]) / plot_per);
   const Real resid  = cumtimeParam - num_per * plot_per;

   if (resid < .001*dt_level[0]) plot_test = 1;
  }
  if (plot_test==0) {
   if ((plot_int > 0)&&(levelstepszero % plot_int == 0))
    plot_test=1;
  }
...
Amr::coarseTimeStep (Real stop_time)
{
    ...
    // Compute new dt.
    //
    Real prev_coarse_dt=dt_level[0];

    if (level_steps[0] > 0)
     amr_level[0].computeNewDt(dt_level,stop_time);

    timeStep(0,cumtime,1,1,prev_coarse_dt,stop_time);
    ...
    int check_test = 0;
    int plot_test = 0;
    long levelstepszero = 0;
    output_checks(cumtime,check_test,plot_test,
      levelstepszero,1); // endofstep=1

    if (check_test == 1) {
     last_checkpoint = levelstepszero;
     checkPoint();
    }
    if (plot_test == 1) {
     last_plotfile = levelstepszero;
     writePlotFile(plot_file_root,levelstepszero);
    }
    is_first_step_after_restart=0;
}
...
Amr::defBaseLevel (Real strt_time)
{
    ...
    //
    // levelbld is a pointer of type *LevelBld and a member of Amr.
    // LevelBld is defined in LevelBld.H
    // The constructor for LevelBld is defined in NSBld.cpp which
    // in turn calls the constructor for NavierStokes. (NavierStokes.cpp)
    // The constructor in NavierStokes is derived from the constructor
    // in AmrLevel (AmrLevel.cpp).  The constructor in AmrLevel allocates
    // the state data. (now, both new and old data are simultaneously defined)
    // 
    // initialize amr_level[0]
    //
...
Amr::regrid (int  lbase,
 ...
 const int start = regrid_level_zero ? 0 : lbase+1;
    ...
    // the destructor explicitly sets parent=0, but it should
    // automatically call the destructor for all of the members of 
    //  amr_level[lev], including Array<StateData> state
    // the destructor for a type of StateData deletes new_data and old_data.
    // 
 ...
 if ((verbose || record_run_info) && ParallelDescriptor::IOProcessor()) {
  for (int lev = start; lev <= finest_level; lev++) {
   ...
   if (verbose) {
    std::cout << "   level " << lev << ": " << numgrids
       << " grids, " << ncells << " cells  = "
       << frac << " % of domain" << std::endl;
   }
   if (record_run_info) {
    runlog << "   level " << lev << ": " << numgrids
    << " grids, " << ncells << " cells  = "
    << frac << " % of domain" << std::endl;
   }
  }
 ...
 if (record_grid_info && ParallelDescriptor::IOProcessor()) {
  ...
  gridlog << "TIME = " << time
    << " : REGRID  with lbase = " << lbase << std::endl;
                    ...
                    int           max_lev)
{
    ...
    for (int lev = min_lev; lev <= max_lev; lev++) {
        ...
        if (lev<max_lev) 
         for (int i = 0; i < BL_SPACEDIM; i++)
          rntot*=amr_level[lev].get_fine_ratio(i);
    }
    os << '\n';
}
...
#undef STRIP
    ...
    for (int levc = max_crse; levc >= lbase; levc--)
    {
        ...
        TagBoxArray tags(old_grids,n_error_buf[levc]+ngrow);
          // initializes the valid regions of "tags"
        int is_old=0;
        amr_level[levc].errorEst(tags,TagBox::CLEAR,TagBox::SET,is_old);
        //
