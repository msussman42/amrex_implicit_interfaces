...
void
Amr::SetDistributionMap (int lev, const DistributionMapping& dmap_in)
{ 
    if (dmap[lev] != dmap_in) dmap[lev] = dmap_in;
}

void
Amr::SetBoxArray (int lev, const BoxArray& ba_in)
{
    if (grids[lev] != ba_in) grids[lev] = ba_in;
}   

void
Amr::ClearDistributionMap (int lev)
{ 
    dmap[lev] = DistributionMapping();
}   

void
Amr::ClearBoxArray (int lev)
{
    grids[lev] = BoxArray();
}   
...
Amr::defBaseLevel (Real strt_time)
{
    ...
    //
    amr_level.set(0,(*levelbld)(*this,0,geom[0],grids[0],dmap[0],strt_time));
                  ...
                  Array<BoxArray>& new_grids)
{
 ...
 for (i = lbase+1; i <= max_crse; i++) {
  ...
  p_n_comp[i].refine(rr_lev[i-1]);
   // grow each box in p_n_comp[i] by n_proper
  ...
  for (int cnt = 1; cnt <= 4; cnt *= 2) {
   ...
   for (int i = lbase; i <= new_finest; i++) {
    ...
    for (int j = 0; j < BL_SPACEDIM; j++) {
     ...
     if ((new_grids[i].size() < NProcs) && 
         (chunk[j]%blocking_factor[i] == 0)) {
...
Amr::bldFineLevels (Real strt_time)
{
 ...
 while (grid_places_done==0) {
  ...
  if (new_finest>finest_level) {
   ...
   amr_level.set(new_finest,a_level);
