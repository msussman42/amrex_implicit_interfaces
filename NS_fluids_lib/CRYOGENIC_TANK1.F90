...
#define SDIM 2
 ...
 contains
  ...
  subroutine INIT_CRYOGENIC_TANK1_MODULE()
   ...
   TANK1_VAPOR_CP = R_UNIV/fort_species_molar_mass(1) + TANK1_VAPOR_CV ! [J∕(kg·K)]
  ...
  ! R_spc = C_{p,spc}-C_{v,spc}
  ! R_unv = C_{p,m} - C_{v,m} 
  ! gamma = C_{p,spc}/C_{v,spc}
  !
  ! p = rho R_spc T 
  !   = rho R_spc x U/C_{v,spc}
  !   = rho (C_{p,spc}-C_{v,spc})/C_{v,spc} U
  !   = rho (gamma-1) U
  !
   ...
   if (num_species_var_in.eq.num_species_var) then
    if (im.eq.2) then
     if (imattype.eq.24) then
      ...
      temperature=internal_energy/C_V_SPC_MIX(massfrac_var(1))
     else
      print *,"imattype= ",imattype
      print *,"imattype invalid"
      stop
  ...
  REAL_T function GAMMA_MIX(X_V)
   ...
   GAMMA_MIX = &
    (X_V * TANK1_VAPOR_CP + (one-X_V)*TANK1_GAS_CP) / &
    (X_V * TANK1_VAPOR_CV + (one-X_V)*TANK1_GAS_CV)
  ...
  subroutine CRYOGENIC_TANK1_PRES(x,t,LS,PRES,nmat)
   ...
   elseif (x(2).lt.TANK1_LIQUID_HEIGHT) then
    PRES=TANK1_INITIAL_MIX_PRESSURE +&
     fort_denconst(2)*(TANK1_LIQUID_HEIGHT-x(2))*(-gravity)
   else
