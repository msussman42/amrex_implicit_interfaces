...
MODULE global
	...
	!--------------------------------
	INTEGER, PARAMETER, PUBLIC	:: CalcPotentials=0, &	! if=1, calc. phi on arb. grid inside needle
	...
	ShowChiAngle=1, &		! if=1, calcs & prints surface angle
	...
	CaseShape=1				!Outer shape. =1, single box, simple pot; =2 ripple box, real pot; 
							!=3, Suvorov test case with electrode on axis; =4 is a 'T' top shape

	REAL*8, PARAMETER, PUBLIC	:: elecPotential=1e7, &	! electrode potential [V]
	fluidPot=-2., &			! base fluid potential [V]
	maxDistStep=1.2e-3, &	! max dist. between evenly spaced points [* major time driver *]
	StepMult=0.20			! multiply number of baseline exterior panels by [* major time driver *]
	...
	HardwireChargeYes=1, &	! if=1, hardwire a charge and see the shape evolve.
		! REQUIRES SKIPPING OVER BEM approach
	InitArySize=8000, &		! max # of points/blob. size of outgoing radblob var. 
	MaxNumInpotIter=3, &	! MS was 6
	MinNumPtsBlob=7 , &		! what is the minimum number of points per blob?
	npoints=10, & ! # pts / panel; # panels / shortest side
		! NOTE: need to make sure 'constants.h:npoints' has the same value as npoints
	PotMeshSize=50, &		! number of grid cells^2 for optional potential evaluation
	...
	HardwireCharge=8.8987e-4, & ! hardwired charge [C] to give 1e9 E field at 8mm
