...
PROGRAM MAIN
	...
	IMPLICIT NONE
  	...
  	! PARAMETERS
	INTEGER, PARAMETER	:: Runit=10, Phiunit=11, CaseLoad=4, LSGridMaxZ=609, LSGridMaxR=305
	...
	REAL*8				:: rlo, zlo, rhi, zhi, dr, dz, elecHeight, r, z, &
		rblob, zblob, radblob2, CosConst, baseExpZ, relsize
	...
	CONTINUE
	! simulation values
	rlo=0.; rhi=0.5; zlo=-0.05; zhi=0.95
	rblob=0.5; zblob=0.5; radblob2=0.25
	elecHeight=1.0; is_symmetric=0
	...
	ENDIF
	relsize=1.*(zhi-zlo)/(rhi-rlo) - 1.*(LSGridMaxZ-1)/(LSGridMaxR-1)
	IF ( ABS(relsize) > eps) THEN
		PRINT *, 'AxiSym4: Grid # relative size mismatch.', relsize
		STOP		
	ENDIF
	...
	CASE(3)	! smooth data set, no bumps
		...
		DO i=1,LSGridMaxR
			...
			DO j=1,LSGridMaxZ
				z = (j+0.5)*dz + zlo
				!phi2(i,j)= 0.5-0.25*cos(2.0*pi*x)-y
	...
	CALL BoundaryForceCalc( NumBlobs, blob_rc, blob_zc, NumPt_blob, MxPt_blob, radBlob, &
		...
		dr, dz, elecHeight )	! formal full version

	PRINT *, 'Done with AxiSym4'
	GOTO 500
