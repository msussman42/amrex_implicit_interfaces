...
20     continue
     ...
     &  fdata,DIMS(fdata),flo,fhi,dt)
      ...
      INTEGER_T init,namlen
      INTEGER_T name(namlen)
      INTEGER_T untin, i
...
#include "probdata.H"
      ...
      return
       ...
       subroutine FORT_OVERRIDE(rz_flag,max_level_front,
     ...
     &   ccdenair,ccdenwater,ccdenvapor,ccdenspread,ccadvbot,
     &   ccviscburn,
...
#include "probdata.H"
     ...
     &   rgasinlet,vinletgas,twall,walltemp,tcenter,denair
       ...
       print *,"fort: denwater,denspread,advbot,viscburn,viscunburn ",
     &   denwater,denspread,advbot,viscburn,viscunburn
...
#include "probdata.H"
      ...
      REAL_T ldata(3,3,3),rval,rlow,rhigh,volsum(32),dxsub(SDIM),newdx(SDIM)
...
c 8 for production runs, 12 for testing curvature
     ...
     &                     ldata(i,j,k),dxin(1))
         ...
         call getvolume(ldata,rz_flag,rlow,rhigh,volfrac)
...
c      call handfree(x,yblob,y,dist)
      ...
      REAL_T x,y,z,dist
      REAL_T NPT,HSB,NID,NOD,CHH,scaleCHH
      REAL_T xx1(maxnumline),yy1(maxnumline),xx2(maxnumline),yy2(maxnumline)
...
#include "probdata.H"
      ...
      subroutine soliddist(x,y,dist,hx,translate,rotate,time)
      IMPLICIT NONE
      REAL_T x,y,z,dist,hx,time,dist1,temprad
      REAL_T rr,xx,yy,zz,aspect,offset,distplate
      REAL_T translate(3),rotate(3),xlarge
...
c      offset=2.54d0
       ...
       if (offset.lt.hx) then
        print *,"radblob2 (thickness plate) too small or base grid too coarse"
        stop
       endif
       distplate=yblob2
...
c      offset=1.1*yblob/two
     ...
     &         (probtype.eq.21)) then
       ...
       zz=yblob
       onlypaddle=0
       call paddlegeom(x,zz,y,translate,rotate,dist,onlypaddle)
      else if (probtype.eq.52) then
       ...
       xsmall=-xblob
       xlarge=xblob+half*xblob
       ysmall=yblob
       ylarge=two*yblob
       call squaredist(x,y-translate(SDIM),
     &    xsmall,xlarge,ysmall,ylarge,dist)
...
#include "probdata.H"
      ...
      hugedist=99999.0
      dist=hugedist
      if ((probtype.eq.1).and.(axis_dir.gt.10)) then
       dist=hugedist
      else if ((probtype.eq.1).or.(probtype.eq.2).or.(probtype.eq.11)) then
       dist=sqrt( (x-xblob)**2 + (y-yblob)**2 )-radblob
...
c      call damdist(x,y,dist,igeom)
     ...
     &         (probtype.eq.62).or.(probtype.eq.58)) then
       dist=hugedist
      else if (probtype.eq.38) then
       dist=hugedist
...
c     ::::: local variables
      INTEGER_T i, j,k,dir,i1,j1,k1,atfront1,atfront2
      REAL_T  x, y,z
      REAL_T  dist1,dist2,xprime,yprime,zprime
      REAL_T  nn(3),dpdn,hx,hy,hz,vel,maxvel
      REAL_T  vofnew(3,3,3),vofold(3,3,3)
...
#include "probdata.H"
      ...
      do i = lo(1),hi(1)
       ...
       if (probtype.eq.50) then
        if (x.gt.twall+0.25) then 
         usolid(i,j,xsolid+1)=rotspeed(1)*(y-position(3))
         usolid(i,j,xsolid+2)=-rotspeed(1)*(x-position(1))
        endif
       else if (probtype.eq.54) then
     ...
     &           vofnew(i1+2,j1+2,k1+2),tfinish)
          call handgeom(xprime,yblob,yprime,
     &           vofold(i1+2,j1+2,k1+2),tstart)
          if (abs(vofnew(i1+2,j1+2,k1+2)).gt.maxvel) then
           maxvel=abs(vofnew(i1+2,j1+2,k1+2))
          endif
          if (abs(vofold(i1+2,j1+2,k1+2)).gt.maxvel) then
           maxvel=abs(vofold(i1+2,j1+2,k1+2))
          endif
         enddo 
         enddo 
         enddo 
         ...
         do j1=-1,1
         do k1=-1,1
          vofnew(i1+2,j1+2,k1+2)=vofnew(i1+2,j1+2,k1+2)/maxvel
          vofold(i1+2,j1+2,k1+2)=vofold(i1+2,j1+2,k1+2)/maxvel
         enddo
         enddo
         enddo
         ...
         if ((atfront1.eq.1).and.(atfront2.eq.1)) then
          nn(1)=(vofnew(3,2,2)-vofnew(1,2,2))/(two*hx)
          nn(2)=(vofnew(2,3,2)-vofnew(2,1,2))/(two*hy)
          nn(3)=(vofnew(2,2,3)-vofnew(2,2,1))/(two*hz)
          dist1=sqrt(nn(1)**2+nn(2)**2+nn(3)**2)
          if (dist1.gt.1.0E-4) then
           maxvel=abs(adv_vel)
           dpdn=-(vofnew(2,2,2)-vofold(2,2,2))/
     &        ((tfinish-tstart)*dist1)
           ...
           do dir=1,SDIM
            usolid(i,j,dir)=usolid(i,j,dir)+dpdn*nn(dir)/dist1
           enddo
          endif
         endif
c atfront
       ...
       if ((probtype.eq.55).and.(axis_dir.eq.4)) then 
        ...
        if (phase.eq.0) then
         ...
         if (axis_dir.eq.5) then
          if (y.le.yblob8) then
           temp(i,j)= walltemp+y*(twater-walltemp)/yblob8
          endif
         endif
...
c bc are associated with DOMAIN boundary conditions.
      subroutine FORT_PERIODICFIX(mf,DIMS(mf),lo,hi,
     &  ngrow,dir,bc,problen,problo,probhi,domlo,domhi,dx,
     &  time,xlo)
      IMPLICIT NONE
      ...
      if ((dir.lt.0).or.(dir.ge.SDIM)) then
       print *,"dir invalid in PERIODIC FIX"
       stop
      endif
      ...
      if (dir.eq.0) then
       ii=1
       call XPOSFILL(mf,DIMS(mf),domlo,domhi,dx,xlo,time,bc)
      else if (dir.eq.1) then
       jj=1
       call YPOSFILL(mf,DIMS(mf),domlo,domhi,dx,xlo,time,bc)
      else
       print *,"dir invalid in PERIODICFIX"
       stop
      endif
     ...
     &    (dir.eq.0)) then
       ...
       do j=lo(2)-ngrow,hi(2)+ngrow
        if (mf(i,j).ge.problo(dir+1)) then
         mf(i,j)=mf(i,j)-problen(dir+1)
        endif
       enddo
       enddo
      endif
     ...
     &    (dir.eq.0)) then
       ...
       do j=lo(2)-ngrow,hi(2)+ngrow
        if (mf(i,j).le.probhi(dir+1)) then
         mf(i,j)=mf(i,j)+problen(dir+1)
        endif
       enddo
       enddo
     ...
     &  pres,temp,extraindex,time,domlo,domhi)
      IMPLICIT NONE
...
c n.eq.xvel+1
     ...
     &             (yblob10.ne.zero)) then
          ...
          else if ((probtype.eq.41).and.(axis_dir.eq.1)) then
           if (abs(x).le.xblob) then
            mf(i,j)=advbot
           else
            mf(i,j)=vinletgas
           endif
        ...
        if ((bc(2,1).eq.EXT_DIR).and.(lo(2)-ngrow.lt.domlo(2))) then
         ...
         do i=lo(1)-ngrow,hi(1)+ngrow
          ...
          if ((probtype.eq.25).and.(axis_dir.gt.0)) then
           ...
           mf(i,j)=mf(i,domlo(2))-(xpos(i,domlo(2),2)-y) 
           if (mf(i,j).ge.zero) then
            mf(i,j)=y
           endif
          else if ((probtype.eq.41).and.(axis_dir.eq.1)) then
           mf(i,j)=abs(x)-xblob     
...
c fine regions where levelsetmap<0
      subroutine levelsetmap(x,dir,dist)
      IMPLICIT NONE
...
#include "probdata.H"
      ...
      if (dir.eq.1) then
       if (probtype.eq.53) then
        if (abs(x-xblob).lt.radblob) then
         dist=zero
        else
       ...
       print *,"dir invalid in levelsetmap"
       stop
      endif
      ...
      subroutine FORT_INITMAP(domlo,domhi,problo,probhi,dx,level,
     &  maxlevel,mapspread)
      IMPLICIT NONE
      ...
      REAL_T xmap(0:8192,SDIM)
      INTEGER_T maphi(SDIM)
      common / globalmap / xmap,maphi

      REAL_T ww(0:8192)
      REAL_T datamap(0:8192)
      REAL_T datamapold(0:8192)
      REAL_T thickness,hsprime,xval,wwsum,wwsumpart
      REAL_T err,maxerr,dist,hx
      INTEGER_T i,iter,dir,numcell

      do dir=1,SDIM
       maphi(dir)=domhi(dir)+1
       hx=dx(dir)
       do i=level+1,maxlevel
        maphi(dir)=maphi(dir)*2
        hx=half*hx
       enddo

       if (maphi(dir).gt.8192) then
        print *,"maphi too big!"
        stop
       endif
       if (domlo(dir).ne.0) then
        print *,"domlo should be 0"
        stop
       endif
       thickness=mapspread*hx
       numcell=maphi(dir)
       do i=0,numcell
        datamap(i)=problo(dir)+i*hx
        datamapold(i)=problo(dir)+i*hx
       enddo
       do iter=1,100
        do i=0,numcell
         xval=datamap(i)
         call levelsetmap(xval,dir,dist)
         ww(i)=one/(hsprime(dist,thickness)+one)
        enddo

        wwsum=zero
        do i=0,numcell-1
         wwsum=wwsum+half*(ww(i)+ww(i+1))
        enddo
        wwsumpart=zero
        do i=0,numcell
         datamap(i)=(probhi(dir)-problo(dir))*wwsumpart/wwsum+problo(dir)
         wwsumpart=wwsumpart+half*(ww(i)+ww(i+1))
        enddo

        maxerr=zero
        do i=0,numcell
         err=abs(datamap(i)-datamapold(i))
         if (err.gt.maxerr) then
          maxerr=err
         endif
        enddo
        if (maxerr.lt.1.0e-7) then
         goto 20
        endif
        do i=0,numcell
         datamapold(i)=datamap(i)
        enddo
        print *,"iter,maxerr ",iter,maxerr
       enddo
20     continue

       do i=0,numcell
        xmap(i,dir)=datamap(i)
       enddo
...
c     ::::: local variables
      INTEGER_T i, j, k
      INTEGER_T ratio, ratioleft, ratioright
      INTEGER_T ileft,iright,jleft,jright,kleft,kright
      REAL_T  x, y, z
      REAL_T  hx, hy, hz
...
#include "probdata.H"
      ...
      enddo
      ratioleft=ratio/2
      if (2*ratioleft.eq.ratio) then
       ratioright=ratioleft
      else
       ratioright=ratioleft+1
      endif
      ...
      do j = lo(2)-1,hi(2)+1
      do i = lo(1)-1,hi(1)+1
       y = xlo(2) + hy*(REAL(j-lo(2))) + half*hy
       x = xlo(1) + hx*(REAL(i-lo(1))) + half*hx

       ileft=i*ratio+ratioleft
       iright=i*ratio+ratioright
       jleft=j*ratio+ratioleft
       jright=j*ratio+ratioright

       if ((i.ge.0).and.(iright.le.maphi(1))) then
        x=half*(xmap(ileft,1)+xmap(iright,1))
       endif
       if ((j.ge.0).and.(jright.le.maphi(2))) then
        y=half*(xmap(jleft,2)+xmap(jright,2))
       endif
     ...
     &	scal,DIMS(scal),translate,rotate,time,levelsolid,xpos)
      IMPLICIT NONE
...
#include "probdata.H"
      ...
      do i = lo(1),hi(1)
       ...
       call soliddist(x,y,scal(i,j,levelsolid+1),
     &   hx,translate,rotate,time)
      enddo
      enddo
     ...
     &	xpos,vel,DIMS(vel),dx,xlo,xhi,phase)
      IMPLICIT NONE
...
c rotate is in radians, positive angle rotates clockwise.
      subroutine paddlegeom(x,y,z,translate,rotate,dist,onlypaddle)
      IMPLICIT NONE

      REAL_T x,y,z,translate(3),rotate(3),dist
...
#include "probdata.H"
      ...
      if ((rotate(2).ne.zero).or.(rotate(3).ne.zero)) then
      ...
      xrot=cos(rotate(1))*xnew-sin(rotate(1))*znew 
      zrot=sin(rotate(1))*xnew+cos(rotate(1))*znew 
      yrot=ynew

      call geteblevel(xrot,yrot,zrot,1,diststar)
      xmin=-50.0
      xmax=twall
      ymin=-50.0
      ymax=50.0
...
c     ymin2=2.75
      ...
      call cubedist(xmin,xmax,ymin,ymax,zmin,zmax,
     &              x,y,z,distcube) 
      call cubedist(xmin1,xmax1,ymin1,ymax1,zmin1,zmax1,
     &              x,y,z,distcube1) 
      call cubedist(xmin2,xmax2,ymin2,ymax2,zmin2,zmax2,
     &              x,y,z,distcube2) 
      call cubedist(xmin3,xmax3,ymin3,ymax3,zmin3,zmax3,
     &              x,y,z,distcube3) 	
      dist=diststar
      if ((dist.gt.distcube).and.(onlypaddle.eq.0)) then
       dist=distcube
      endif
      if ((dist.gt.distcube1).and.(onlypaddle.eq.0)) then
       dist=distcube1
      endif
      if ((dist.gt.distcube2).and.(onlypaddle.eq.0)) then
       dist=distcube2
      endif
      if ((dist.gt.distcube3).and.(onlypaddle.eq.0)) then
       dist=distcube3
      endif
      return
      end
...
#include "probdata.H"
      ...
      do i = lo(1), hi(1)
       ...
       tagflag=0
       if ((probtype.eq.16).or.(probtype.eq.6)) then
        print *,"fix VFRACERROR for this probtype"
        stop
       endif
     ...
     &       (y.le.hy) ) then
       ...
       else if ((level.ge.maxlevelf).and.(tcenter.gt.zero)) then
     ...
     &      (abs(y-yblob10).le.radblob10)) then
       ...
       tag(i,j) = cvmgt(set,tag(i,j),(tagflag.eq.1) )
      enddo
      enddo
...
#include "probdata.H"
     ...
     &           ((axis_dir.eq.0).or.(axis_dir.eq.5))) then
              adv(i,j)=tvapor
             endif
	    enddo
	 enddo
...
c at the center of the GHOST CELL and not ON THE EDGE.
	...
	p(i,j) = -p(ihi,j)
     ...
     &  vel,DIMS(vel),dir,xpos,DIMS(xpos),lo,hi)
      IMPLICIT NONE
      INTEGER_T lo(SDIM),hi(SDIM)
      INTEGER_T DIMDEC(vel)
      INTEGER_T DIMDEC(xpos)
