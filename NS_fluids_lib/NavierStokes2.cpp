...
void NavierStokes::CELL_GRID_ELASTIC_FORCE(int im_elastic) {
 ...
 debug_ngrow(CELL_DEN_BASE_MF,1,5);
...
void NavierStokes::increment_KE(Real beta) {
 ...
 int ncomp_state=STATECOMP_STATES+num_materials*num_state_material+
...
#endif
{
 ...
 ns_reconcile_d_num(LOOP_INC_TEMP);
...
#endif
{
 ...
 ns_reconcile_d_num(LOOP_VELMAC_TO_CELL_GENERAL);
...
#endif
{
   ...
   ns_reconcile_d_num(LOOP_FACE_GRADIENTS);
  } // tileloop=0..1
 } // dir
...
#endif
{
 ...
 ns_reconcile_d_num(LOOP_FILL_BOUNDARY_TENSOR);
...
#endif
{
 ...
 ns_reconcile_d_num(LOOP_OVERRIDDE_FAB_TYPE);
...
#endif
{
   ...
   ns_reconcile_d_num(LOOP_CROSSTERM);
...
#endif
{
   ...
   ns_reconcile_d_num(LOOP_PRESGRAD_MAC);
...
void NavierStokes::allocate_physics_vars() {
 ...
 new_localMF_if_not_exist(CELL_DEN_MF,1,1,-1); // ncomp,ngrow,dir
  ...
  //CELL_DEN_BASE_MF contains (1/rho_base)   (no added mass)
  // ncomp,ngrow,dir
 new_localMF_if_not_exist(CELL_DEN_BASE_MF,1,1,-1); 
...
#endif
{
 ...
 ns_reconcile_d_num(LOOP_SEMIREFINEVOF);
...
// face_frac=0 if presbc<> interior or exterior dirichlet.
   ...
   FArrayBox& cden_base_fab=(*localMF[CELL_DEN_BASE_MF])[mfi]; // 1/rho_base 
   ...
   fort_init_physics_vars(
    ...
    cden_base_fab.dataPtr(),
    ARLIM(cden_base_fab.loVect()),ARLIM(cden_base_fab.hiVect()),
  ...
  ns_reconcile_d_num(LOOP_INIT_PHYSICS_VARS);
 } // isweep
 ...
 if ((fab_verbose==1)||(fab_verbose==3)) {
   ...
   for (int dir=0;dir<AMREX_SPACEDIM;dir++) {
    ...
    ns_reconcile_d_num(LOOP_TECPLOT_DEBUG_CURV_POSTPROC);
...
#endif
{
   ...
   ns_reconcile_d_num(LOOP_INITSOLIDTEMP);
  } else
...
#endif
{
  ...
  ns_reconcile_d_num(LOOP_ADDGRAVITY);
 }  // dir=0..sdim-1
...
#endif
{
  ...
  ns_reconcile_d_num(LOOP_INIT_POTENTIAL);
...
#endif
{
  ...
  ns_reconcile_d_num(LOOP_POTGRAD_TO_MAC);
 } // tileloop
 } // dir
 synchronize_flux_register(operation_flag,spectral_loop);
 } // spectral_loop
...
#endif
{
 ...
 ns_reconcile_d_num(LOOP_METRICS);
...
#endif
{
   ...
   ns_reconcile_d_num(LOOP_RENORMALIZE_PRESCRIBE);
...
#endif
{
   ...
   ns_reconcile_d_num(LOOP_MOVE_PARTICLE_CONTAINER);
...
//  in: LEVELSET_3D.F90
 ...
 ns_reconcile_d_num(LOOP_PURGEFLOTSAM);
...
// structure.
 ...
 ns_reconcile_d_num(LOOP_PARTICLE_GRID);
...
void NavierStokes::output_zones(
 ...
 // is no tiling.
    ...
    ns_reconcile_d_num(LOOP_CELLGRID);
...
#endif
{
    for (MFIter mfi(*velmfminus,use_tiling); mfi.isValid(); ++mfi) {
     ...
     // declared in: NavierStokes.cpp
    ns_reconcile_d_num(LOOP_CELLGRID_AS_IS);
    ParallelDescriptor::Barrier();
...
// is no tiling.
   ...
   ns_reconcile_d_num(LOOP_CELLGRID_SANITY);
...
void NavierStokes::VOF_Recon_resize(int ngrow,int dest_mf) {
 ...
 if ((ngrow<1)||(ngrow>ngrow_distance))
  amrex::Error("ngrow invalid");
 if (localMF[dest_mf]->nComp()!=num_materials*ngeom_recon)
  amrex::Error("localMF[dest_mf]->nComp()!=num_materials*ngeom_recon");
 ...
 if (localMF[dest_mf]->nGrow()==ngrow) {
  ...
  MultiFab::Copy(*slopes_mf,*localMF[dest_mf],0,0,num_materials*ngeom_recon,0);
  delete_localMF(dest_mf,1);
  new_localMF(dest_mf,num_materials*ngeom_recon,ngrow,-1); //sets values to 0.0
  MultiFab::Copy(*localMF[dest_mf],*slopes_mf,0,0,num_materials*ngeom_recon,0);
  ...
  for (int i=0;i<num_materials*ngeom_recon;i++)
   ...
   //scomp=0
  PCINTERP_fill_borders(dest_mf,ngrow,0,num_materials*ngeom_recon,
    State_Type,scompBC_map);
...
#endif
{
 ...
 ns_reconcile_d_num(LOOP_SLOPE_RECON);
...
#endif
{
 ...
 ns_reconcile_d_num(LOOP_BUILD_MASKSEM);
...
#endif
{
 ...
 ns_reconcile_d_num(LOOP_EOS_PRESSURE);
...
#endif
{
 ...
 ns_reconcile_d_num(LOOP_GETSHEAR);
...
#endif
{
 ...
 ns_reconcile_d_num(LOOP_PRESSURE_INDICATOR);
...
#endif
{
 ...
 ns_reconcile_d_num(LOOP_ADVECTIVE_PRESSURE);
...
#endif
{
  ...
  ns_reconcile_d_num(LOOP_DERVISCOSITY);
...
#endif
{
   ...
   ns_reconcile_d_num(LOOP_DERTURBVISC);
...
#endif
{
  ...
  ns_reconcile_d_num(LOOP_DERCONDUCTIVITY);
...
#endif
{
  ...
  ns_reconcile_d_num(LOOP_DERMAGTRACE);
