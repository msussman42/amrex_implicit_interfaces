...
void NavierStokes::diffuse_hoop(int idx_vel,int idx_thermal,
 ...
 debug_ngrow(FSI_GHOST_MF,1,811);
 ...
 if (localMF[idx_thermal]->nComp()!=num_materials_vel)
  BoxLib::Error("localMF[idx_thermal]->nComp() invalid");

 debug_ngrow(idx_force,1,812);
 if (localMF[idx_force]->nComp()!=nsolveMM)
  BoxLib::Error("localMF[idx_force]->nComp() invalid");
...
void NavierStokes::mom_force(int idx_neg_mom_force,int update_state) {
 ...
 if (localMF[idx_neg_mom_force]->nComp()!=nsolveMM) {
  ...
  BoxLib::Error("localMF[idx_neg_mom_force]->nComp() invalid");
 }
 ...
 for (MFIter mfi(U_new,use_tiling); mfi.isValid(); ++mfi) {
  ...
  if (update_state==1) { 
   if (fabs(cur_time_slab-prev_time_slab-dt_slab)>1.0E-5) {
    ...
    BoxLib::Error("slab time bust2");
   }
  } else if (update_state==0) {
   if ((cur_time_slab<0.0)||
       (prev_time_slab<0.0)||
       (dt_slab<=0.0))
    BoxLib::Error("cur_time_slab, prev_time_slab, or dt_slab invalid6");
  } else
   BoxLib::Error("update_state invalid");
...
void NavierStokes::viscous_boundary_fluxes(
 ...
 int nsolve) {
  ...
  if (project_option==3) { // viscosity
   ...
   BoxLib::Error("project_option not supported");
  ...
  for (int dir=0;dir<BL_SPACEDIM;dir++) {
   ...
   if (dir==0)
    ...
    BoxLib::Error("dir invalid viscous boundary fluxes");

   if (fluxdir->nComp()!=nsolveMM_FACE)
    BoxLib::Error("conserve_fluxes invalid ncomp");
...
void NavierStokes::combine_state_variable(
 ...
 if ((project_option==0)||
  ...
  BoxLib::Error("project_option invalid80");
 ...
 if (hydrate_flag==1) {
  if (num_species_var!=1)
   BoxLib::Error("num_species_var invalid");
 }
 ...
 if ((project_option==1)||   // initial projection
  ...
  BoxLib::Error("project_option invalid in combine_state_variable");
 } else if (project_option==0) {    // regular projection
  nsolve=1;
  if (combine_flag!=2)
   BoxLib::Error("combine_flag invalid");
  num_materials_combine=1;
  if (update_flux!=1)
   BoxLib::Error("update_flux invalid");
  ...
  BoxLib::Error("project_option invalid81");
 ...
 if (ncomp_check!=nsolveMM) {
  ...
  BoxLib::Error("nsolveMM invalid 313");
 }
 ...
 if (LEVEL_COMBINE->nGrow()<1) {
  ...
  BoxLib::Error("LEVEL_COMBINE->nGrow()<1");
 }
 if (LEVEL_COMBINE->nComp()!=nmat*(BL_SPACEDIM+1))
  BoxLib::Error("LEVEL_COMBINE->nComp()!=nmat*(BL_SPACEDIM+1)");
 ...
 if ((combine_flag==0)||  // FVM -> GFM
  ...
  if (signed_distance->nComp()!=nmat*(1+BL_SPACEDIM))
   BoxLib::Error("signed_distance->nComp() invalid");
 ...
 if ((signed_distance->nGrow()==1)||
  ...
  BoxLib::Error("signed_distance->nGrow() invalid");
 }
 ...
 if ((update_flux==0)&&(combine_idx==-1)&&
  ...
  BoxLib::Error("update_flux or combine_idx invalid");
   ...
   for (int dir=0;dir<BL_SPACEDIM;dir++) {
    ...
    if (combine_idx==-1) {
     ...
     if (project_option==0) {
      ...
      BoxLib::Error("project_option invalid82");
     ...
     BoxLib::Error("combine_idx invalid");

    if (face_mf->nComp()!=nsolveMM_FACE)
     BoxLib::Error("face_mf->nComp() invalid");
...
#endif
{
  ...
  if (combine_idx==-1) {
   ...
   if (cell_mf->nComp()!=nstate)
    BoxLib::Error("cell_mf->nComp() invalid");
  } else if (combine_idx>=0) {
...
#endif
{
  for (MFIter mfi(S_new,use_tiling); mfi.isValid(); ++mfi) {
   ...
   if (listbc.size()!=nsolveMM*BL_SPACEDIM*2)
    BoxLib::Error("listbc.size() invalid");
  ...
  if ((combine_idx==-1)&&(hflag==0)) {
   ...
   BoxLib::Error("combine_idx or hflag invalid");
...
void NavierStokes::diffusion_heating(int source_idx,int idx_heat) {
 ...
 if (localMF[source_idx]->nComp()!=nsolveMM)
  BoxLib::Error("localMF[source_idx]->nComp() invalid");

 if (localMF[idx_heat]->nComp()!=num_materials_scalar_solve)
  BoxLib::Error("localMF[idx_heat]->nComp()!=num_materials_scalar_solve");
