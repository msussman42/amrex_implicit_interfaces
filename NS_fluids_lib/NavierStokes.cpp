...
NavierStokes::read_params ()
{
    ...
    growth_angle_primary_mat.resize(num_interfaces);
    growth_angle_tertiary_mat.resize(num_interfaces);
    growth_angle_secondary_mat.resize(num_interfaces);
    for (int i=0;i<num_interfaces;i++) {
     growth_angle[i]=0.0;
    ...
    pp.queryAdd("growth_angle_primary_mat",
		growth_angle_primary_mat,num_interfaces);
    pp.queryAdd("growth_angle_tertiary_mat",
		growth_angle_tertiary_mat,num_interfaces);
    pp.queryAdd("growth_angle_secondary_mat",
		growth_angle_secondary_mat,num_interfaces);
...
int NavierStokes::FSI_material_exists_presvel() {
 ...
 for (int im=0;im<num_materials;im++) {
  if (FSI_flag[im]==FSI_SHOELE_PRESVEL) {
   local_flag=1;
  } else if (FSI_flag_valid(im)==1) {
   // do nothing
  } else {
  ...
  if ((FSI_operation==0)||  // initialize nodes
   ...
   if (elements_generated==0) {
    ...
    if (FSI_operation==0) { 
     ...
     im_critical_start=-1-num_materials;
    } else
     amrex::Error("FSI_operation invalid");
   ...
   fort_headermsg(
     ...
     &cur_time, 
     &dt, 
     FSI_refine_factor.dataPtr(), // 1,...,num_materials
...
#endif
{
  ...
  ns_reconcile_d_num(LOOP_ASSIMILATE_Q_FROM_PART,"assimilate_Q_from_particles");
...
NavierStokes::particle_tensor_advection_update() {
 ...
 std::string local_caller_string="particle_tensor_advection_update";
...
#endif
{
   ...
   ns_reconcile_d_num(LOOP_UPDATE_PARTICLE_TENSOR,
     "particle_tensor_advection_update");
