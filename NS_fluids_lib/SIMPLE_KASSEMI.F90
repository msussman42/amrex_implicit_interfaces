...
REAL_T C_pG,k_G,den_G,lambda
  ...
  if (TANK_MK_R_UNIV.eq.fort_R_Palmore_Desjardins) then
   ...
   print *,"TANK_MK_R_UNIV.ne.fort_R_Palmore_Desjardins"
   stop
  endif
...
!  TANK_MK_GAS_CP = fort_stiffCP(2) ![J∕(kg·K)]
  ...
  call SIMPLE_KASSEMI_GetDiffusionLayer(l_verification)

  use_t=1
  dummy_x=zero
  dummy_t=zero
  call SIMPLE_KASSEMI_TEMPorMASSFRAC( &
...
return
end subroutine SIMPLE_KASSEMI_LS

! initial velocity is zero
subroutine SIMPLE_KASSEMI_VEL(x,t,LS,VEL,velsolid_flag,dx,nmat)
use probcommon_module
IMPLICIT NONE
...
return 
end subroutine SIMPLE_KASSEMI_VEL
...
subroutine EOS_KASSEMI_MK(rho,massfrac_var, &
...
subroutine SOUNDSQR_KASSEMI_MK(rho,massfrac_var, &
 ...
 if (num_species_var_in.eq.num_species_var) then
  if (im.eq.2) then
   if (imattype.eq.24) then
     ! a = sqrt(gamma R_sp T) = sqrt(gamma p/rho)
    call EOS_KASSEMI_MK(rho,massfrac_var, &
 ...
 return
end subroutine SOUNDSQR_KASSEMI_MK

subroutine INTERNAL_KASSEMI_MK(rho,massfrac_var, &
 ...
 return
end subroutine INTERNAL_KASSEMI_MK

subroutine TEMPERATURE_KASSEMI_MK(rho,massfrac_var, &
 ...
 if (num_species_var_in.eq.num_species_var) then
  if (im.eq.2) then
   if ((imattype.eq.24).or.(imattype.eq.0)) then 
    ...
    print *,"imattype= ",imattype
    print *,"imattype invalid TEMPERATURE_KASSEMI_MK"
    stop
   endif
  else
 ...
 return
end subroutine TEMPERATURE_KASSEMI_MK
...
subroutine SIMPLE_KASSEMI_PRES(x,t,LS,PRES,nmat)
use probcommon_module
IMPLICIT NONE
...
return 
end subroutine SIMPLE_KASSEMI_PRES


subroutine SIMPLE_KASSEMI_DiffusionLayer(l,f) 
 use probcommon_module
 use global_utility_module
 IMPLICIT NONE
...
end subroutine SIMPLE_KASSEMI_DiffusionLayer

subroutine SIMPLE_KASSEMI_GetDiffusionLayer(l)
 ...
 a = 0.0d0
 b = 5.0d0
 call SIMPLE_KASSEMI_DiffusionLayer(a,fa)
 call SIMPLE_KASSEMI_DiffusionLayer(b,fb)
 ...
 do while (iter.LT.100)
  c = (a+b)/2.0d0
  call SIMPLE_KASSEMI_DiffusionLayer(c,fc)
  iter = iter + 1
  call SIMPLE_KASSEMI_DiffusionLayer(a,fa)
 ...
 enddo ! iter.LT.100
 l=c
end subroutine SIMPLE_KASSEMI_GetDiffusionLayer
...
subroutine SIMPLE_KASSEMI_TEMPorMASSFRAC( &
...
end subroutine SIMPLE_KASSEMI_TEMPorMASSFRAC

subroutine SIMPLE_KASSEMI_STATE(x,t,LS,STATE,bcflag,nmat,nstate_mat)
use probcommon_module
use global_utility_module
IMPLICIT NONE
...
return
end subroutine SIMPLE_KASSEMI_STATE



subroutine SIMPLE_KASSEMI_ASSIMILATE( &
...
return
end subroutine SIMPLE_KASSEMI_ASSIMILATE



 ! dir=1..sdim  side=1..2
subroutine SIMPLE_KASSEMI_LS_BC(xwall,xghost,t,LS, &
...
if ((dir.ge.1).and.(dir.le.SDIM).and. &
 ...
 call SIMPLE_KASSEMI_LS(xghost,t,LS,nmat)
else
...
return
end subroutine SIMPLE_KASSEMI_LS_BC


 ! dir=1..sdim  side=1..2 veldir=1..sdim
subroutine SIMPLE_KASSEMI_VEL_BC(xwall,xghost,t,LS, &
...
if ((dir.ge.1).and.(dir.le.SDIM).and. &
 ...
 call SIMPLE_KASSEMI_VEL(xghost,t,LS,local_VEL,velsolid_flag,dx,nmat)
...
return
end subroutine SIMPLE_KASSEMI_VEL_BC

 ! dir=1..sdim  side=1..2
subroutine SIMPLE_KASSEMI_PRES_BC(xwall,xghost,t,LS, &
...
if ((dir.ge.1).and.(dir.le.SDIM).and. &
 ...
 call SIMPLE_KASSEMI_PRES(xghost,t,LS,PRES,nmat)
...
return
end subroutine SIMPLE_KASSEMI_PRES_BC

 ! dir=1..sdim  side=1..2
subroutine SIMPLE_KASSEMI_STATE_BC(xwall,xghost,t,LS, &
...
if ((istate.ge.1).and. &
 ...
 call SIMPLE_KASSEMI_STATE(xghost,t,LS,local_STATE,local_bcflag, &
...
return
end subroutine SIMPLE_KASSEMI_STATE_BC

subroutine SIMPLE_KASSEMI_HEATSOURCE(im,VFRAC,time,x, &
     xsten,nhalf,temp, &
...
end subroutine SIMPLE_KASSEMI_SUMINT


end module SIMPLE_KASSEMI_module
