...
stop
      ...
      if (initialize_flag.eq.0) then
       ...
       print *,"initialize_flag invalid: ",initialize_flag
       stop
      endif
...
! c T^new - c T^old = (dt A LL/V) dS/dt = LL * dF 
      ...
      if (nucleation_flag.eq.0) then
       ...
       print *,"nucleation_flag invalid: ",nucleation_flag
       stop
      endif
             ...
             if (LL(ireverse).ne.zero) then
              ...
              if ((is_rigid(im).eq.1).or. &
               ...
               if ((abs(LShere(im_source)).le.two*dxmax).and. &
                ...
                if ((test_k_source.eq.zero).and. &
                    (test_k_dest.eq.zero)) then
                 !do nothing
                else if (((test_k_source.ge.zero).and. &
                          (test_k_dest.gt.zero)).or. &
                         ((test_k_source.gt.zero).and. &
                          (test_k_dest.ge.zero))) then
                 ...
                 if (found_path.eq.1) then  
                  ...
                  call prepare_normal(nrmCP,mag,SDIM)
...
! 1 erg=1 g(cm/s)^2=10^{-7} kg(m/s)^2 Joules
                 ...
                 print *,"test_k_source incorrect? ",test_k_source
                 print *,"test_k_dest incorrect? ",test_k_dest
                 stop
                endif
