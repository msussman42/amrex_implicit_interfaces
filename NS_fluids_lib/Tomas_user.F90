...
!yblob5=R
 ...
 contains
  ...
  subroutine INIT_Tomas_MODULE() 
   IMPLICIT NONE
   return
  end subroutine INIT_Tomas_MODULE
...
REAL_T function DIST_FINITE_CYLHEAD(P,R_cyl,H_bot,H_top)
 ...
 if((H_bot.le.z).and.(z.le.H_top)) then
  ...
  subroutine Tomas_INIT_LS(x,t,LS)
   ...
   REAL_T thickness,Radius,LambdaWave,piin, HelixLength, scalelength, cylinderHeight,LStmp1,LStmp2,BodyStart
   INTEGER_T im
   ...
   if (SDIM.eq.3) then
    if ((num_materials.eq.3).and.(probtype.eq.36)) then
...
!        LS(im) = 0.5- sqrt( (cos(tt+t)-x(1))**2 + (sin(tt+t)-x(2))**2 + (tt-x(3))**2 )  !without z velocity
   ...
   return
  end subroutine Tomas_INIT_LS


  subroutine Tomas_INIT_STATE(x,t,LS,STATE)
   use probcommon_module
   IMPLICIT NONE
   ...
   return
  end subroutine Tomas_INIT_STATE

  !********************************************************   

  subroutine Tomas_INIT_VEL(x,t,LS,VEL)
   use probcommon_module
   IMPLICIT NONE
   ...
   enddo
   im_solid_Tomas=2
   if (LS(im_solid_Tomas).ge.zero) then
    VEL(2)=-x(3)
    VEL(3)=x(2)
    VEL(1)=0.0
   else if (LS(im_solid_Tomas).lt.zero) then
    VEL(adv_dir) = adv_vel
   else
   ...
   return 
  end subroutine Tomas_INIT_VEL

  !********************************************************  

  subroutine Tomas_INIT_PRES(x,t,LS,PRES)
   use probcommon_module
   IMPLICIT NONE
   ...
   return 
  end subroutine Tomas_INIT_PRES
  ...
  ! Velocity        : Dirichlet  (*)
  ! Level set       : Dirichlet  (*) (Tomas => switched to Extrap)
  ! Volume fraction : Dirichlet  (*) (Tomas => switched to Extrap)
  ! Pressure        : N  (*)
  ...
  subroutine Tomas_LS_BC(xwall,xghost,t,LS, &
   LS_in,dir,side,dx)
   use probcommon_module
   IMPLICIT NONE
   ...
   if ((dir.ge.1).and.(dir.le.SDIM).and. &
    ...
    call Tomas_INIT_LS(xghost,t,LS)
   ...
   return
  end subroutine Tomas_LS_BC

  !******************************************************* 

  subroutine Tomas_VEL_BC(xwall,xghost,t,LS, &
   ...
   if ((dir.ge.1).and.(dir.le.SDIM).and. &
    ...
    call Tomas_INIT_VEL(xghost,t,LS,local_VEL)
   ...
   return
  end subroutine Tomas_VEL_BC

  !******************************************************* 

  subroutine Tomas_PRES_BC(xwall,xghost,t,LS, &
   ...
   if ((dir.ge.1).and.(dir.le.SDIM).and. &
    ...
    call Tomas_INIT_PRES(xghost,t,LS,PRES)
   ...
   return
  end subroutine Tomas_PRES_BC

  !******************************************************* 

  subroutine Tomas_STATE_BC(xwall,xghost,t,LS, &
   ...
   if ((istate.ge.1).and. &
    ...
    call Tomas_INIT_STATE(xghost,t,LS,local_STATE)
    ibase=(im-1)*num_state_material
   ...
   return
  end subroutine Tomas_STATE_BC
end module Tomas_module
