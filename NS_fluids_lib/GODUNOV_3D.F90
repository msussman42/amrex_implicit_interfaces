...
! (grad * u^{T})_{ij}=\partial u_{j}/\partial x_{i}
      ...
      do i=growlo(1),growhi(1)
       ...
       if (local_mask.eq.0) then
        ...
        IEforce=zero
        do refine_loop=1,ENUM_NUM_REFINE_DENSITY_TYPE
        ...
        enddo !refine_loop=1,ENUM_NUM_REFINE_DENSITY_TYPE

        IEforce=IEforce/ENUM_NUM_REFINE_DENSITY_TYPE
...
! fort_vfrac_split code starts here
         ...
         do while (istate.le.num_state_material)
          ...
          if (istate.eq.ENUM_DENVAR+1) then ! Density
           ...
           if ((is_compressible_mat(im).eq.0).or. &
               (material_conservation_form(im).eq.0)) then
             ! den * T
            conserve(D_DECL(icrse,jcrse,kcrse), &
              fine_offset+CISLCOMP_STATES+tempcomp_data)= &
                   dencore(im)*local_temperature
                    ...
                    (material_conservation_form(im).eq.1)) then
            ...
            print *, &
             "is_compressible_mat or material_conservation_form invalid"
            print *,"im=",im
            print *,"is_compressible_mat ",is_compressible_mat(im)
            print *,"material_conservation_form ", &
            ...
            if (volint.gt.zero) then
             ...
             do im=1,num_materials
              ...
              do veldir=1,SDIM
                ! fluid materials tessellate the domain.
               if (is_rigid(im).eq.0) then
                 !mom2 comes from conserve(fine_offset+veldir)
                 !conserve(fine_offset+veldir) comes from [xyz]mac_old
       ...
       do im=1,num_materials
        ...
        if ((is_compressible_mat(im).eq.0).or. &
         ...
         print *,"im,material_conservation_form: ", &
              im,material_conservation_form(im)
         stop
        endif
      ...
      subroutine fort_vfrac_split_cell_centered( &
       nprocessed, &
       tid, &
       sato_model_spec_id, &
       density_floor, &
       ...
       dombc, &
       domlo,domhi) &
      bind(c,name='fort_vfrac_split_cell_centered')
...
! fort_vfrac_split code starts here
      ...
      if ((num_materials_viscoelastic.ge.1).and. &
          (num_materials_viscoelastic.le.num_materials)) then
       print *,"expecting num_materials_viscoelastic=0"
       stop
      else if (num_materials_viscoelastic.eq.0) then
       !do nothing
      else
       print *,"num_materials_viscoelastic invalid:fort_vfrac_split_cc ", &
          num_materials_viscoelastic
       stop
      endif
      ...
      if ((num_materials_compressible.ge.1).and. &
          (num_materials_compressible.le.num_materials)) then
       print *,"expecting num_materials_compressible=0"
       stop
      else if (num_materials_compressible.eq.0) then
       !do nothing
      else
       print *,"num_materials_compressible invalid:fort_vfrac_split_cc ", &
           num_materials_compressible
       stop
      endif
      ...
      do im=1,num_materials
       ...
       if (is_elastic(im).eq.1) then
        print *,"expecting is_elastic(im)==0"
        stop
       else if (is_elastic(im).eq.0) then
        !do nothing
       else
       ...
       if (fort_material_type(im).eq.0) then
        ...
        ! do nothing
                ...
                (fort_material_type(im).le.MAX_NUM_EOS)) then
        print *,"expecting all incompressible materials"
        stop
       else
        print *,"fort_material_type invalid"
        stop
      ...
      do icrse=growlo(1),growhi(1)
       ...
       if (levelrz.eq.COORDSYS_CARTESIAN) then
        ...
        print *,"levelrz invalid fort_vfrac_split_cc (vel_coarse) ",levelrz
        stop
       endif
       ...
       KE=zero
       do veldir=1,SDIM
        conserve(D_DECL(icrse,jcrse,kcrse),veldir)=vel_coarse(veldir)
        KE=KE+vel_coarse(veldir)**2
       enddo
       KE=half*KE
       ...
       do im=1,num_materials
         ...
         if (is_compressible_mat(im).eq.0) then
          ...
          print *,"no compressible materials in this routine"
          stop
         else
          print *,"is_compressible_mat(im) invalid ", &
            im,is_compressible_mat(im)
       ...
       do im=1,num_materials
         ...
         do while (istate.le.num_state_material)
          ...
          if (istate.eq.ENUM_DENVAR+1) then ! Density
           dencomp_data=(im-1)*num_state_material+ENUM_DENVAR+1
           conserve(D_DECL(icrse,jcrse,kcrse), &
             CISLCOMP_STATES+dencomp_data)=dencore(im)
           istate=istate+1
          else if (istate.eq.ENUM_TEMPERATUREVAR+1) then ! Temperature
           tempcomp_data=(im-1)*num_state_material+ENUM_TEMPERATUREVAR+1
           local_temperature=den(D_DECL(icrse,jcrse,kcrse),tempcomp_data)
           if ((is_compressible_mat(im).eq.0).or. &
            ...
            conserve(D_DECL(icrse,jcrse,kcrse),CISLCOMP_STATES+tempcomp_data)= &
                   dencore(im)*local_temperature
                    ...
                    (material_conservation_form(im).eq.1)) then
            print *,"compressible not allowed"
            stop
           else
            print *, &
             "is_compressible_mat or material_conservation_form invalid"
      ...
      do icrse=growlo(1),growhi(1)
          ...
          if (volint.gt.zero) then
           ...
           do im=1,num_materials
            ...
            if (is_compressible_mat(im).eq.0) then
             ...
             print *,"expecting all incomp fluids"
             stop
            else
             print *,"is_compressible_mat(im) invalid ", &
              im,is_compressible_mat(im)
       ...
       do im=1,num_materials
        if (is_compressible_mat(im).eq.0) then
         ...
         print *,"not expecting compressible"
         stop
        else
         print *,"is_compressible_mat(im) invalid ",im, &
                 is_compressible_mat(im)
      ...
      return
      end subroutine fort_vfrac_split_cell_centered
...
!STATECOMP_MOF
      ...
      integer, INTENT(in) :: tid
      integer, INTENT(in) :: dir 
      integer, INTENT(in) :: ncomp_interface 
      integer, INTENT(in) :: LS_base_comp
      ...
      real(amrex_real), INTENT(in), target :: &
         improved(DIMV(improved),ncomp_interface)
      real(amrex_real), INTENT(in), target :: &
         improved_vel(DIMV(improved_vel))
      real(amrex_real), INTENT(in), target :: &
         standard(DIMV(standard),ncomp_interface)
      real(amrex_real), INTENT(in), target :: &
      ...
      real(amrex_real) F_left(num_materials)
      real(amrex_real) F_right(num_materials)
      ...
      if (ncomp_interface.ge.compressible_base_comp) then
       !do nothing
      else
       print *,"ncomp_interface invalid ",ncomp_interface
       stop
      endif
      if (LS_base_comp.eq.STATE_NCOMP) then
       !do nothing
      else
       print *,"LS_base_comp invalid ",LS_base_comp
       stop
      endif
      if (elastic_base_comp.eq.STATE_NCOMP+num_materials*(1+SDIM)) then
       !do nothing
      else
       print *,"elastic_base_comp invalid"
       stop
      endif
      if (compressible_base_comp.ge.elastic_base_comp) then
       !do nothing
      else
       print *,"compressible_base_comp invalid"
       stop
      endif
      ...
      call checkbound_array(fablo,fabhi,improved_ptr,1,-1)
      call checkbound_array1(fablo,fabhi,improved_vel_ptr,0,vel_dir)
      call checkbound_array(fablo,fabhi,standard_ptr,1,-1)
      call checkbound_array1(fablo,fabhi,standard_vel_ptr,0,vel_dir)
      call checkbound_array(fablo,fabhi,snew_ptr,1,-1)
      call checkbound_array(fablo,fabhi,lsnew_ptr,1,-1)
      call checkbound_array1(fablo,fabhi,macnew_ptr,0,vel_dir)
      ...
      if (dir.eq.-1) then
       ...
       do i=growlo(1),growhi(1)
        ...
        do im=1,num_materials 
         ...
         LS_standard(im)=standard(D_DECL(i,j,k),LS_base_comp+im)
         LS_improved(im)=improved(D_DECL(i,j,k),LS_base_comp+im)
         F_standard(im)=standard(D_DECL(i,j,k),STATECOMP_MOF+vofcompraw)
         F_improved(im)=improved(D_DECL(i,j,k),STATECOMP_MOF+vofcompraw)
         ...
         if ((is_elastic(im).eq.1).or.(is_rigid(im).eq.1)) then
          ...
          do dir_local=1,SDIM+1
           mofnew(vofcomprecon+dir_local-1)= &
             improved(D_DECL(i,j,k),STATECOMP_MOF+vofcompraw+dir_local-1)
          enddo
          local_LS(im)=LS_improved(im)

          do dir_local=1,num_state_material
           snew(D_DECL(i,j,k), &
             STATECOMP_STATES+(im-1)*num_state_material+dir_local)= &
            improved(D_DECL(i,j,k), &
             STATECOMP_STATES+(im-1)*num_state_material+dir_local)
          enddo
          ...
          do dir_local=1,SDIM+1
           mofnew(vofcomprecon+dir_local-1)= &
             standard(D_DECL(i,j,k),STATECOMP_MOF+vofcompraw+dir_local-1)
          enddo
          ...
          do dir_local=1,num_state_material
           snew(D_DECL(i,j,k), &
             STATECOMP_STATES+(im-1)*num_state_material+dir_local)= &
            standard(D_DECL(i,j,k), &
             STATECOMP_STATES+(im-1)*num_state_material+dir_local)
          enddo

         else
          print *,"is_elastic invalid ",im,is_elastic(im)
          print *,"or is_rigid invalid ",im,is_rigid(im)
          stop
         endif
        enddo !im=1,num_materials
        ...
        do im=1,num_materials
         ...
         vofcomprecon=(im-1)*ngeom_recon+1
         do dir_local=1,SDIM+1
          snew(D_DECL(i,j,k),STATECOMP_MOF+vofcompraw+dir_local-1)= &
             mofnew(vofcomprecon+dir_local-1)
         enddo
        ...
        do im=1,num_materials 
         vofcompraw=(im-1)*ngeom_raw+1
         F_improved(im)=snew(D_DECL(i,j,k),STATECOMP_MOF+vofcompraw)
        enddo
        call get_primary_material_VFRAC(F_improved,im_primary)
        do dir_local=1,SDIM+1
         if ((is_elastic(im_primary).eq.1).or. &
             (is_rigid(im_primary).eq.1)) then
          snew(D_DECL(i,j,k),dir_local)=improved(D_DECL(i,j,k),dir_local)
         else if ((is_elastic(im_primary).eq.0).and. &
                  (is_rigid(im_primary).eq.0)) then
          snew(D_DECL(i,j,k),dir_local)=standard(D_DECL(i,j,k),dir_local)
         else
          print *,"is_rigid or is_elastic invalid"
          stop
         endif
        enddo !dir_local=1,SDIM+1
       ...
       do i=growlo(1),growhi(1)
        ...
        do im=1,num_materials 
         ...
         if ((is_rigid(im).eq.1).or.(is_elastic(im).eq.1)) then
          F_left(im)=improved(D_DECL(i-ii,j-jj,k-kk),STATECOMP_MOF+vofcompraw)
          F_right(im)=improved(D_DECL(i,j,k),STATECOMP_MOF+vofcompraw)
         else if ((is_rigid(im).eq.0).and.(is_elastic(im).eq.0)) then
          F_left(im)=standard(D_DECL(i-ii,j-jj,k-kk),STATECOMP_MOF+vofcompraw)
          F_right(im)=standard(D_DECL(i,j,k),STATECOMP_MOF+vofcompraw)
         else
          print *,"is_rigid or is_elastic invalid"
          stop
         endif
        ...
        call get_primary_material_VFRAC(F_left,im_primary_left)
        call get_primary_material_VFRAC(F_right,im_primary_right)

        if ((is_rigid(im_primary_left).eq.1).or. &
            (is_rigid(im_primary_right).eq.1).or. &
            (is_elastic(im_primary_left).eq.1).or. &         
            (is_elastic(im_primary_right).eq.1)) then
         macnew(D_DECL(i,j,k))=improved_vel(D_DECL(i,j,k))
        else if ((is_rigid(im_primary_left).eq.0).and. &
                 (is_rigid(im_primary_right).eq.0).and. &
                 (is_elastic(im_primary_left).eq.0).and. &         
                 (is_elastic(im_primary_right).eq.0)) then
         macnew(D_DECL(i,j,k))=standard_vel(D_DECL(i,j,k))
        else
         print *,"is_rigid or is_elastic invalid"
         stop
        endif
      ...
      if (ntsat.eq.EXTRAP_NCOMP_TSAT) then
       ...
       print *,"nstat invalid fort_combinevel ",ntsat
       stop
      endif

      if ((level.lt.0).or.(level.gt.finest_level)) then
       print *,"level invalid fort_combinevel ",level,finest_level
       stop
      endif
      if ((hydrate_flag.ne.0).and.(hydrate_flag.ne.1)) then
       print *,"hydrate_flag invalid fort_combinevel ",hydrate_flag
       stop
      endif
      if ((nsolve.ne.1).and. &
          (nsolve.ne.STATE_NCOMP_VEL)) then
       print *,"nsolve invalid fort_combinevel ",nsolve
       stop
      endif

      if (nstate_main.ne.STATECOMP_STATES+ &
          num_materials*(num_state_material+ngeom_raw)+1) then
       print *,"nstate_main invalid fort_combinevel (1): ",nstate_main
       stop
      endif
      if (nstate_main.ne.STATE_NCOMP) then
       print *,"nstate_main invalid fort_combinevel (2): ",nstate_main
       stop
      endif

      if ((ncomp_cell.ne.num_materials_combine).and. &
          (ncomp_cell.ne.nstate_main)) then
       print *,"ncomp_cell invalid fort_combinevel ",ncomp_cell
       stop
      endif
   
      if (num_state_base.ne.2) then
       print *,"num_state_base invalid fort_combinevel ",num_state_base
       stop
      endif
       ...
       if (mask_test.eq.1) then
        ...
        do im=1,num_materials
         ...
         if ((local_volume.ge.-EPS1).and. &
          ...
          print *,"local_volume invalid fort_combinevel: ",local_volume
          stop
         endif
         ...
         if (test_density.gt.zero) then
          ...
          print *,"test_density invalid fort_combinevel: ",test_density
          stop
         endif
         ...
         if (test_temperature.gt.zero) then
          ...
          print *,"test_temperature invalid fort_combinevel: ",test_temperature
          stop
         endif
         ...
         do ispec=1,num_species_var
          ...
          if (massfrac_parm(ispec).ge.zero) then
           ...
           print *,"massfrac_parm(ispec) invalid fort_combinevel: ", &
              massfrac_parm(ispec)
           stop
          endif
         ...
         if (DeDT.gt.zero) then
          ...
          print *,"DeDT must be positive fort_combinevel: ",DeDT
          stop
         endif
         ...
         if (project_option.eq.SOLVETYPE_VISC) then ! viscosity
          ...
          if (hflag.eq.0) then !inhomogeneous solid velocity
           if (cell_temperature(im).ge.zero) then
            ! do nothing
           else
            print *,"cell_temperature(im) must be positive: combinevel"
            print *,"cell_temperature(im)=",cell_temperature(im)
            print *,"im=",im
            print *,"cellcomp=",cellcomp
            stop
           endif
          else if (hflag.eq.1) then !homogeneous solid velocity
           if ((cell_temperature(im).ge.zero).or. &
               (cell_temperature(im).le.zero)) then
            ! do nothing
           else
            print *,"cell_temperature(im) is NaN: ", &
               cell_temperature(im)
            stop
           endif
          else
           print *,"hflag invalid3 fort_combinevel hflag=",hflag
           stop
          endif
         ...
         if (project_option.eq.SOLVETYPE_HEAT) then
          state_mass_average=state_mass_average+ &
             local_mass*DeDT*cell_temperature(im)
         else if ((project_option.ge.SOLVETYPE_SPEC).and. & ! species
                  (project_option.lt.SOLVETYPE_SPEC+num_species_var)) then
          state_mass_average=state_mass_average+ &
             local_mass*cell_temperature(im)
        ...
        if (total_vol_cell.gt.zero) then
         ...
         print *,"total_vol_cell invalid fort_combinevel: ",total_vol_cell
         stop
        endif
        ...
        if (mass_sum.gt.zero) then
         ...
         print *,"mass_sum invalid fort_combinevel: ",mass_sum
         stop
        endif
        ...
        do im=1,num_materials
          !FSI_flag_local.eq.FSI_PRESCRIBED_PROBF90
          !FSI_flag_local.eq.FSI_PRESCRIBED_NODES
          !FSI_flag_local.eq.FSI_SHOELE_CTML
          !FSI_flag_local.eq.FSI_ICE_NODES_INIT
          !FSI_flag_local.eq.FSI_FLUID_NODES_INIT
        ...
        if (project_option.eq.SOLVETYPE_VISC) then ! viscosity
         ...
         if (combine_flag.eq.FILL_EMPTY_CELL) then !vfrac<VOFTOL_MATERIAL
          ...
          if (scomp(1).ne.STATECOMP_VEL) then
           print *,"scomp invalid fort_combinevel: ",scomp(1)
           stop
          endif
          if (ncomp(1).ne.nsolve) then
           print *,"ncomp(1) invalid fort_combine_vel: ",ncomp(1)
           print *,"nsolve=",nsolve
           stop
          endif
           ...
           if (hflag.eq.0) then !inhomogeneous solid velocity
            if (im_solid_vel.eq.0) then
             ...
             if (cellcomp.eq.1) then
              ...
              print *,"cellcomp invalid fort_combinevel ",cellcomp
              stop
             endif

            else
             print *,"im_solid_vel invalid fort_combinevel ",im_solid_vel
             stop
            endif
           else if (hflag.eq.1) then !homogeneous solid velocity
            vsol(cellcomp)=zero
           else
            print *,"hflag invalid2 fort_combinevel hflag=",hflag
            stop
           endif
          ...
          if (LS_clamped.ge.zero) then
           ...
           print *,"LS_clamped is NaN fort_combinevel ",LS_clamped
           stop
          endif
          ...
          if ((is_solid_cell.ge.1).and. &
           ...
           print *,"is_solid_cell invalid fort_combinevel: ",is_solid_cell
           stop
          endif
         ...
         do im_crit=1,num_materials
          ...
          if (combine_flag.eq.GFM_TO_FVM) then !center->centroid
           ...
           if (abs(cell_temperature(1)).le.VOFTOL_MATERIAL) then
            ...
            if (abs(cell_temperature(1)- &
                    state_mass_average).le.VOFTOL_MATERIAL) then
             !do nothing
            else
             print *,"cell_temperature or state_mass_average invalid:", &
               im_crit, &
               cell_temperature(1), &
               state_mass_average
             stop
         ...
         if (weight_sum.gt.zero) then
          ...
          print *,"weight_sum invalid fort_combinevel: ",weight_sum
          stop
         endif 
         ...
         if (hflag.eq.0) then !inhomogeneous solid velocity
          ...
          print *,"hflag invalid4 fort_combinevel hflag=",hflag
          stop
         endif
         ...
         do im=1,num_materials
          ...
          if ((cell_vfrac(im).le.one-VOFTOL_MATERIAL).and. &
           ...
           if (((im_primary.eq.im).and. &
            ...
            if ((im_primary.ne.im).and. &
                (combine_flag.eq.GFM_TO_FVM)) then

             newcell(D_DECL(i,j,k),im)=state_mass_average

            else if ((im_primary.eq.im).or. &
                     (combine_flag.eq.FVM_TO_GFM)) then
             ...
             do im_crit=1,num_materials
              ...
              if (Tcenter(im_crit).ge.zero) then
               ...
               print *,"Tcenter(im_crit) invalid ",im_crit,Tcenter
               stop
              endif
              if (project_option.eq.SOLVETYPE_HEAT) then
                     ...
                     if ((cell_vfrac(im).ge.VOFTOL_MATERIAL).and. &
                      ...
                      if (project_option.eq.SOLVETYPE_HEAT) then
                       ...
                       if ((Tgamma_STATUS.eq.1).or. &
                        ...
                        print *,"Tgamma_STATUS invalid ",Tgamma_STATUS
                        stop
                       endif
                      else
                      ...
                      if ((Tgamma_STATUS.eq.1).or. &
                       ...
                       if (LL.lt.zero) then ! freezing
                        ...
                        print *,"LL invalid; fort_combinevel ",LL
                        stop
                       endif
                      ...
                      print *,"cell_vfrac invalid ",cell_vfrac
                      stop
                     endif
              ...
              if (is_rigid(im_primary).eq.0) then
               ...
               if ((abs(LS_source).le.two*dxmax).and. &
                ...
                if (LS_dest.ge.zero) then
                 ...
                 print *,"LS_dest or LS_source invalid ",LS_dest,LS_source
                 stop
                endif
             ...
             print *,"im_primary invalid ",im_primary
             print *,"or combine_flag invalid ",combine_flag
             stop
            endif
      ...
      do i=growlo(1),growhi(1)
       ...
       if (at_RZ_face.eq.1) then
        ...
        if (is_rigid(imL).eq.1) then 
         ...
         print *,"is_rigid(imL) invalid ",imL,is_rigid(imL)
         stop
        endif
        ...
        if (is_rigid(imR).eq.1) then
         ...
         print *,"is_rigid(imR) invalid ",imR,is_rigid(imR)
         stop
        endif
...
! JTAN edge centered variable
      ...
      real(amrex_real), INTENT(in), target :: &
         ughostx(DIMV(ughostx),nparts_ghost*SDIM) 
      real(amrex_real), pointer :: ughostx_ptr(D_DECL(:,:,:),:)
      real(amrex_real), INTENT(in), target :: &
         ughosty(DIMV(ughosty),nparts_ghost*SDIM) 
      real(amrex_real), pointer :: ughosty_ptr(D_DECL(:,:,:),:)
      real(amrex_real), INTENT(in), target :: &
         ughostz(DIMV(ughostz),nparts_ghost*SDIM) 
      ...
      if ((level.lt.0).or.(level.gt.finest_level)) then
       print *,"level invalid in fort_assimilate"
       stop
      endif
      if (num_state_base.ne.2) then
       print *,"num_state_base invalid"
      ...
      if ((level.lt.0).or.(level.gt.finest_level)) then
       print *,"level invalid in fort_default_evec"
       stop
      endif
      if (num_state_base.ne.2) then
       print *,"num_state_base invalid"
