...
stop
      ...
      subroutine derive_density( &
       ...
       override_density,massdepart,im,nmat,density, &
       for_hydro)
      use global_utility_module
      ...
      if ((voldepart.le.VOFTOL*voltotal_depart).or. &
          (voltarget.le.VOFTOL*voltotal_depart).or. &
          (fort_material_type(im).eq.999).or. &
       ...
       if (fort_material_type(im).eq.0) then
        ...
        else if (for_hydro.eq.1) then

         if (override_density(im).eq.0) then
          density=fort_denconst(im)
         else if ((override_density(im).eq.1).or. &
                  (override_density(im).eq.2)) then
          density=massdepart/voldepart
         else
          print *,"override_density invalid"
           ...
           if (mag.gt.zero) then
            ...
            if (mag.gt.zero) then
             ...
             if (sinthetaACT.gt.zero) then
              if (costhetaACT.ge.zero) then !  0< theta<90
               dist_to_CL=abs(LS_fluid(im_primary_image))/sinthetaACT
              else if (costhetaACT.le.zero) then ! 90<theta<180
               dist_to_CL=abs(LS_fluid(im_primary_image))
              else
      ...
      if ((slopedir.ge.0).and.(slopedir.lt.SDIM)) then
       call growntileboxMAC(tilelo,tilehi,fablo,fabhi, &
        igridlo,igridhi,ngrow-1,slopedir)
      else
       print *,"slopedir invalid"
       stop
      endif
...
! u_x,v_x,w_x, u_y,v_y,w_y, u_z,v_z,w_z;  
      ...
      if (nsolve*num_materials_vel.ne.nsolveMM) then
       print *,"nsolveMM invalid 3918" 
       stop
      endif
      if (ntensor.ne.SDIM*SDIM) then
       print *,"ntensor invalid"
...
! u_x,v_x,w_x, u_y,v_y,w_y, u_z,v_z,w_z;  
      ...
      REAL_T velnew(DIMV(velnew),SDIM*num_materials_vel)
      ...
      do k=growlo(3),growhi(3)
       ...
       do im=1,nmat
        if (num_materials_scalar_solve.eq.1) then
         ...
         T_local(im)=T_local(im)+ &
          dt*DeDTinverse(D_DECL(i,j,k),im+1)*heat_source_local(im)
        else
      ...
      INTEGER_T growlo(3),growhi(3)
      INTEGER_T bfact
      REAL_T gpfab(DIMV(gpfab), &
        num_materials_vel*SDIM)
      REAL_T divfab(DIMV(divfab), &
        num_materials_vel*nsolve)
      REAL_T hoopfab(DIMV(hoopfab), &
        num_materials_vel*nsolve)
      ...
      do k=growlo(3),growhi(3)
       ...
       do partid=1,nparts
        ...
        if ((FSI_flag(im_part).eq.2).or. & ! prescribed solid CAD
         ...
         vofcomp=num_materials_vel*(SDIM+1)+nmat*num_state_material+ &
            (im_part-1)*ngeom_raw
      ...
      if (SDIM.eq.3) then
       klostencil=-ngrow_extrap
       khistencil=ngrow_extrap
      else if (SDIM.eq.2) then
       klostencil=0
       khistencil=0
      else
      ...
      return
      end subroutine FORT_EXTRAPTENSOR
      ...
      call growntileboxMAC(tilelo,tilehi,fablo,fabhi,growlo,growhi,0,dirnormal)
         ...
         if (num_materials_scalar_solve.eq.1) then
          ...
          if (adjust_temperature.eq.-1) then
           ...
           if (delta_coeff.gt.zero) then
            ...
            coeff_TSAT=coeff_TSAT/local_vol
            ...
            if (adjust_temperature.eq.1) then
             ...
             T_adjust=(original_coeff*T_test+coeff_TSAT)/ &
...
! Q units: J/(m^2 s)
      ...
      subroutine FORT_RENORM_MDOT( &
       mdotplus,mdotminus,mdotcount, &
       level,finest_level, &
       nmat,nten, &
       tilelo,tilehi, &
       fablo,fabhi, &
      ...
      REAL_T mdotplus,mdotminus,mdotcount
      INTEGER_T level,finest_level
      INTEGER_T nmat,nten
      INTEGER_T tilelo(SDIM),tilehi(SDIM)
      INTEGER_T fablo(SDIM),fabhi(SDIM)
      ...
      if ((level.lt.0).or.(level.gt.finest_level)) then
       print *,"level invalid in renorm mdot"
       stop
      endif
      if (num_state_base.ne.2) then
       print *,"num_state_base invalid"
      ...
      call checkbound(fablo,fabhi,DIMS(maskcov),0,-1,1269)
      call checkbound(fablo,fabhi,DIMS(mdot),0,-1,1269)

      if (mdotcount.gt.zero) then
       alpha=(mdotplus+mdotminus)/mdotcount
      else if (mdotcount.eq.zero) then
       alpha=zero
      else
       print *,"mdotcount invalid"
       stop
      endif
      ...
      do k=growlo(3),growhi(3)
       ...
       local_mask=NINT(maskcov(D_DECL(i,j,k)))
       if (local_mask.eq.1) then
        mdot_temp=mdot(D_DECL(i,j,k))
        if (mdot_temp.ne.zero) then
         mdot(D_DECL(i,j,k))=mdot_temp-alpha
        endif
       else if (local_mask.eq.0) then
        ! do nothing
       else
      ...
      return
      end subroutine FORT_RENORM_MDOT
...
! VFRAC_SPLIT code starts here
      ...
      REAL_T u_xsten_accept(-1:1,SDIM)
      REAL_T u_xsten_contain(-1:1,SDIM)
      REAL_T u_xsten_grid_contain(-1:1,SDIM)
      REAL_T u_xsten_grid_part(-1:1,SDIM)
      ...
      INTEGER_T idonate_MAC,jdonate_MAC,kdonate_MAC
      INTEGER_T idonate_CELL,jdonate_CELL,kdonate_CELL
      INTEGER_T icrse,jcrse,kcrse
      INTEGER_T ipart,jpart,kpart
      INTEGER_T ipart_sten,jpart_sten,kpart_sten
      INTEGER_T icell_sten,jcell_sten,kcell_sten
      INTEGER_T ileft,jleft,kleft
      ...
      REAL_T mofdata_depart(recon_ncomp)
      REAL_T mofdata_target(recon_ncomp)
      REAL_T snew_hold(ncomp_state)
      REAL_T tennew_hold(ntensor)
      REAL_T dencore(nmat)
      REAL_T newLS(nmat)
      ...
      REAL_T volmat_depart(nmat)
      REAL_T volmat_target(nmat)
      REAL_T multi_volume_depart(nmat)
      REAL_T multi_volume_target(nmat)
      REAL_T multi_cen_depart(SDIM,nmat)
      REAL_T multi_cen_target(SDIM,nmat)
      REAL_T multi_area_depart(nmat)
      REAL_T multi_area_target(nmat)
      REAL_T newcen(SDIM,nmat)
      REAL_T veldata(nc_bucket)
...
! VFRAC_UNSPLIT code starts here
      ...
      nmax=POLYGON_LIST_MAX ! in: VFRAC_UNSPLIT
      ...
      if ((ngrow.eq.1).or.(ngrow.eq.2)) then
       ! do nothing
      else
       print *,"ngrow invalid VFRAC_UNSPLIT"
       stop
      endif

      if ((ngrow_mac_old.eq.0).or. &
          (ngrow_mac_old.eq.1)) then
       ! do nothing
      else
       print *,"ngrow_mac_old invalid VFRAC_UNSPLIT"
       stop
      endif
      ...
      do u_im=1,nmat
       ...
       if (fort_material_type(u_im).eq.0) then
        ! do nothing
       ...
       if ((density_floor(u_im).lt.zero).or. &
           (density_floor(u_im).ge.fort_denconst(u_im))) then
        print *,"density_floor invalid"
        stop
       endif
       if ((density_ceiling(u_im).le.zero).or. &
           (density_ceiling(u_im).lt.fort_denconst(u_im))) then
        print *,"density_ceiling invalid"
        stop
       endif
       if ((temperature_primitive_variable(u_im).ne.0).and. &
           (temperature_primitive_variable(u_im).ne.1)) then
        print *,"temperature_primitive_variable invalid"
        stop
       endif
       if (added_weight(u_im).le.zero) then
        print *,"added_weight invalid"
        stop
       endif

       if ((fort_material_type(u_im).eq.0).or. &
           (is_rigid(nmat,u_im).eq.1).or. &
           (fort_material_type(u_im).eq.999)) then
        if (temperature_primitive_variable(u_im).ne.1) then
         print *,"temperature_primitive_variable(u_im) invalid"
         stop
        endif
       else if ((fort_material_type(u_im).gt.0).and. &
                (is_rigid(nmat,u_im).eq.0).and. &
                (fort_material_type(u_im).ne.999)) then
        if ((temperature_primitive_variable(u_im).eq.0).or. &
            (temperature_primitive_variable(u_im).eq.1)) then
         ! do nothing
        else
         print *,"temperature_primitive_variable(u_im) invalid"
         stop
        ...
        print *,"fort_material_type(u_im) or is_rigid invalid"
        stop
       endif

       if ((override_density(u_im).ne.0).and. &
           (override_density(u_im).ne.1).and. &
           (override_density(u_im).ne.2)) then
        print *,"override_density invalid"
        stop
       endif
      ...
      if (levelrz.eq.0) then
       ...
       print *,"levelrz invalid vfrac unsplit"
       stop
      endif
      ...
      call checkbound(fablo,fabhi,DIMS(unode),ngrow,0,121)
      call checkbound(fablo,fabhi,DIMS(vnode),ngrow,1,121)
      call checkbound(fablo,fabhi,DIMS(wnode),ngrow,SDIM-1,121)
      ...
      if (face_flag.eq.1) then
       ...
       do veldir=1,SDIM
        ...
        do kcrse=growlo(3),growhi(3)
         ...
         nhalf=1
         call gridstenMAC_level(u_xsten_MAC,icrse,jcrse,kcrse, &
           level,nhalf,veldir)
         ...
         if (levelrz.eq.0) then
          ...
          if ((u_xsten_MAC(0,1).le.VOFTOL*dx(1)).and. &
              (veldir.eq.1)) then
           check_accept=0
          endif
         else if (levelrz.eq.3) then
         ...
         if (check_accept.eq.1) then
          ...
          if ((maskunsplitleft.eq.1).or. &
           ...
           do iside=-1,1,2
            ...
            xlow=u_xsten_contain(-1,veldir)
            xhigh=u_xsten_contain(1,veldir)
            xmid=u_xsten_contain(0,veldir)

            if ((xhigh.gt.xmid).and.(xmid.gt.xlow)) then

             if (veldir.eq.1) then
              do knode1=k1lo,k1hi,2
              do jnode1=-1,1,2
               do udir=1,SDIM
                ulow=velnode(D_DECL(-1,jnode1,knode1),udir)
                uhigh=velnode(D_DECL(1,jnode1,knode1),udir)
                if (iside.eq.-1) then
                 ulow=((xhigh-xmid)*ulow+(xmid-xlow)*uhigh)/(xhigh-xlow)
                 velnode(D_DECL(-1,jnode1,knode1),udir)=ulow
                else if (iside.eq.1) then
                 uhigh=((xhigh-xmid)*ulow+(xmid-xlow)*uhigh)/(xhigh-xlow)
                 velnode(D_DECL(1,jnode1,knode1),udir)=uhigh
                else
                 print *,"iside invalid"
                 stop
                endif
              ...
              do inode1=-1,1,2
               do udir=1,SDIM
                ulow=velnode(D_DECL(inode1,-1,knode1),udir)
                uhigh=velnode(D_DECL(inode1,1,knode1),udir)
                if (iside.eq.-1) then
                 ulow=((xhigh-xmid)*ulow+(xmid-xlow)*uhigh)/(xhigh-xlow)
                 velnode(D_DECL(inode1,-1,knode1),udir)=ulow
                else if (iside.eq.1) then
                 uhigh=((xhigh-xmid)*ulow+(xmid-xlow)*uhigh)/(xhigh-xlow)
                 velnode(D_DECL(inode1,1,knode1),udir)=uhigh
                else
                 print *,"iside invalid"
                 stop
                endif
              ...
              do inode1=-1,1,2
               do udir=1,SDIM
                ulow=velnode(D_DECL(inode1,jnode1,-1),udir)
                uhigh=velnode(D_DECL(inode1,jnode1,1),udir)
                if (iside.eq.-1) then
                 ulow=((xhigh-xmid)*ulow+(xmid-xlow)*uhigh)/(xhigh-xlow)
                 velnode(D_DECL(inode1,jnode1,-1),udir)=ulow
                else if (iside.eq.1) then
                 uhigh=((xhigh-xmid)*ulow+(xmid-xlow)*uhigh)/(xhigh-xlow)
                 velnode(D_DECL(inode1,jnode1,1),udir)=uhigh
                else
                 print *,"iside invalid"
                 stop
                endif
             ...
             print *,"xlow,xhigh or xmid invalid"
             stop
            endif
            ...
            do imac_sten=-1,1
            do jmac_sten=-1,1
            do kmac_sten=k1lo,k1hi
             idonate_MAC=icrse+imac_sten
             jdonate_MAC=jcrse+jmac_sten
             kdonate_MAC=kcrse+kmac_sten

             ileft_sten=idonate_MAC-iii   
             jleft_sten=jdonate_MAC-jjj   
             kleft_sten=kdonate_MAC-kkk   
             iright_sten=idonate_MAC
             jright_sten=jdonate_MAC
             kright_sten=kdonate_MAC

             do iside_sten=-1,1,2

              iside_part_sten=-iside_sten
             
              ! control volume left of face
              if (iside_sten.eq.-1) then
               ipart_sten=ileft_sten
               jpart_sten=jleft_sten
               kpart_sten=kleft_sten
 
              ! control volume right of face
              else if (iside_sten.eq.1) then
               ipart_sten=iright_sten
               jpart_sten=jright_sten
               kpart_sten=kright_sten
              else
               print *,"iside_sten invalid"
               stop
              endif

              nhalf=1
              call gridsten_level(u_xsten_grid_contain, &
                ipart_sten,jpart_sten,kpart_sten, &
                level,nhalf)

              nhalf=1
              call CISBOXHALF(u_xsten_grid_part,nhalf, &
               xlo,dx,ipart_sten,jpart_sten,kpart_sten, &
               iside_part_sten,veldir, &
               bfact,level, &
               volcell_sten,cencell_sten,SDIM)
 
              if (volcell_sten.le.zero) then
               print *,"volcell_sten invalid"
               stop
              endif
              ...
              do id_tet=1,ntetbox
               call extract_tet(xdepartnode,tempdatanode,xtri,datatri, &
                id_tet,symmetry_flag,SDIM)
               call extract_tet(xtargetnode,tempdatanode, &
                xtargettri,datatri,id_tet,symmetry_flag,SDIM)

                ! xtri is a tetrahedra from the departure region.
                ! u_xsten_grid_part is a subset of u_xsten_grid_contain
               nhalf=1
               call intersect_cube(xtri, &
                u_xsten_grid_part,nhalf, &
                geom_xtetlist_old(1,1,1,tid+1), &
                geom_xtetlist(1,1,1,tid+1), &
                nmax, &
                n_tet_in_intersect,nmax,SDIM)

               if (n_tet_in_intersect.gt.0) then
                ...
                do udir=1,SDIM
                 dxgrid(udir)=u_xsten_grid_contain(1,udir)- &
                              u_xsten_grid_contain(-1,udir)
                 if (dxgrid(udir).le.zero) then
                  print *,"u_xsten_grid_contain became corrupt" 
                  stop
                 endif 
                enddo ! udir=1..sdim
                ...
                do udir=1,SDIM
                 ...
                 do udir2=1,SDIM
                  u_xsten_departmap(0,udir)= &
                   u_xsten_departmap(0,udir)+ &
                   coeff(udir,udir2)*u_xsten_grid_contain(0,udir2)
                 enddo ! udir2=1..sdim
                ...
                do u_im=1,nmat
                 vofcomp=(u_im-1)*ngeom_recon+1
                 mofdata_depart(vofcomp)= &
                  PLICSLP(D_DECL(ipart_sten,jpart_sten,kpart_sten), &
                          vofcomp) 
                 do udir=1,SDIM 
                  mofdata_depart(vofcomp+udir)= &
                   PLICSLP(D_DECL(ipart_sten,jpart_sten,kpart_sten), &
                           vofcomp+udir)
                 enddo
                 mofdata_depart(vofcomp+SDIM+1)= &
                  PLICSLP(D_DECL(ipart_sten,jpart_sten,kpart_sten), &
                          vofcomp+SDIM+1) !ord
                 mofdata_depart(vofcomp+2*SDIM+2)= &
                  PLICSLP(D_DECL(ipart_sten,jpart_sten,kpart_sten), &
                          vofcomp+2*SDIM+2) 
                 do udir=1,SDIM
                  nn(udir)= &
                   PLICSLP(D_DECL(ipart_sten,jpart_sten,kpart_sten), &
                           vofcomp+SDIM+1+udir) 
                  mofdata_depart(vofcomp+SDIM+1+udir)=nn(udir)
                 enddo ! udir

                 do udir=1,ngeom_recon
                  mofdata_target(vofcomp+udir-1)= &
                    mofdata_depart(vofcomp+udir-1)
                 enddo
                 ...
                 ! from above: nmap=A^{-T}n
                 do udir=1,SDIM
                  nnmap(udir)=zero
                  do udir2=1,SDIM
                   nnmap(udir)=nnmap(udir)+ASINV(udir2,udir)*nn(udir2)
                  enddo
                  mofdata_target(vofcomp+SDIM+1+udir)=nnmap(udir)
                 enddo ! udir
                enddo ! u_im=1..nmat

                do n_tet_index=1,n_tet_in_intersect

                 ! xinttri is in the departure region; xinttri is
                 ! part of the intersection of xtri with u_xsten_grid_part.
                 do ivert=1,SDIM+1
                 do udir=1,SDIM
                  xinttri(ivert,udir)= &
                    geom_xtetlist(ivert,udir,n_tet_index,tid+1)
                 enddo
                 enddo
                 ...
                 ! find volumes within u_xsten_accept/xmaptri
                 ! u_xsten_grid_contain is mapped to u_xsten_departmap 
                 ! xinttri is mapped to xmaptri
                 shapeflag=1  
                 tessellate=0
                 nhalf=1
                 call multi_get_volume_grid( &
                  ...
                  mofdata_target, &
                  u_xsten_accept,nhalf, &
                  xmaptri, &
                  multi_volume_target,multi_cen_target,multi_area_target, &
                  geom_xtetlist_uncapt(1,1,1,tid+1),  &
                  nmax, &
                  nmax, &
                 ...
                 call multi_get_volume_grid( &
                  ...
                  u_xsten_grid_part,nhalf, &
                  xinttri, &
                  multi_volume_depart,multi_cen_depart,multi_area_depart, &
                  geom_xtetlist_uncapt(1,1,1,tid+1),  &
                  nmax, &
                  nmax, &
                  nmat,SDIM,shapeflag,102)
                 ...
                 do u_im=1,nmat
                  dencomp_data=(u_im-1)*num_state_material+1
                  donate_data= &
                   conserve(D_DECL(ipart_sten,jpart_sten,kpart_sten), &
                            iden_base+dencomp_data)
                  massdepart_mom=donate_data*added_weight(u_im)

                  if (massdepart_mom.le.zero) then
                   print *,"density invalid in vfrac unsplit 1"
                   print *,"ipart_sten,jpart_sten,kpart_sten ", &
                     ipart_sten,jpart_sten,kpart_sten
                   print *,"u_im ",u_im
                   print *,"donate_data ",donate_data
                   print *,"massdepart_mom ",massdepart_mom
                  ...
                  mom2(veldir)= &
                      multi_volume_depart(u_im)*massdepart_mom*donate_data

                  massdepart_mom=massdepart_mom*multi_volume_depart(u_im)

                  veldata(iden_mom_base+u_im)= &
                   veldata(iden_mom_base+u_im)+massdepart_mom

                  momcomp=(u_im-1)*SDIM+veldir
                  veldata(momcomp)=veldata(momcomp)+mom2(veldir) 
                 enddo ! u_im=1,..,nmat 

                enddo ! n_tet_index - traversing triangles in intersection 
               else if (n_tet_in_intersect.eq.0) then
                ! do nothing
               else
                print *,"n_tet_in_intersect invalid"
                stop
               endif 
              enddo ! id_tet=1..ntetbox

             enddo ! iside_sten=-1,1,2
            enddo ! kmac_sten
            enddo ! jmac_sten
            enddo ! imac_sten
            ...
            do u_im=1,nmat
             ...
             if (is_prescribed(nmat,u_im).eq.0) then
             ...
             else if (is_prescribed(nmat,u_im).eq.1) then
              ! do nothing
             else
              print *,"is_prescribed(nmat,u_im) invalid"
              stop
             endif
      ...
      do kcrse=growlo(3),growhi(3)
       ...
       if (maskcell.eq.1) then
        ...
        nhalf=1
        call gridsten_level(u_xsten_crse,icrse,jcrse,kcrse,level,nhalf)
        ...
        if (levelrz.eq.0) then
         ...
         if (u_xsten_crse(0,1).le.VOFTOL*dx(1)) then
          check_accept=0
          print *,"icrse invalid"
          stop
         endif
        ...
        if (check_accept.eq.1) then 
         ...
         Imaskunsplit=NINT(maskunsplit(D_DECL(icrse,jcrse,kcrse)))

         if (Imaskunsplit.eq.1) then
          ...
          nhalf=1
          call CISBOX(u_xsten_accept,nhalf, &
           xlo,dx,icrse,jcrse,kcrse, &
           bfact,level, &
           volcell_accept,cencell_accept,SDIM)
          ...
          do icell_sten=-1,1
          do jcell_sten=-1,1
          do kcell_sten=k1lo,k1hi
           idonate_CELL=icrse+icell_sten
           jdonate_CELL=jcrse+jcell_sten
           kdonate_CELL=kcrse+kcell_sten

           nhalf=1
           call CISBOX(u_xsten_grid_contain,nhalf, &
             xlo,dx,idonate_CELL,jdonate_CELL,kdonate_CELL, &
             bfact,level, &
             volcell_sten,cencell_sten,SDIM)
           ...
           do id_tet=1,ntetbox
            ...
            if (n_tet_in_intersect.gt.0) then
             ...
             do u_im=1,nmat
              vofcomp=(u_im-1)*ngeom_recon+1
              mofdata_depart(vofcomp)= &
               PLICSLP(D_DECL(idonate_CELL,jdonate_CELL,kdonate_CELL), &
                       vofcomp) 
              do udir=1,SDIM 
               mofdata_depart(vofcomp+udir)= &
                PLICSLP(D_DECL(idonate_CELL,jdonate_CELL,kdonate_CELL), &
             ...
             do n_tet_index=1,n_tet_in_intersect
              ...
              do ivert=1,SDIM+1
              do udir=1,SDIM
               xinttri(ivert,udir)=geom_xtetlist(ivert,udir,n_tet_index,tid+1)
              enddo
              enddo
              ...
              if (LS_voltotal_depart.le.zero) then
               ...
               do u_im=1,nmat 
                vofcomp=(u_im-1)*ngeom_recon+1
                print *,"u_im,multi_volume_depart ",u_im, &
                 multi_volume_depart(u_im)
                print *,"u_im,multi_volume_target ",u_im, &
                 multi_volume_target(u_im)
                print *,"u_im,vfrac ",u_im,mofdata_depart(vofcomp)
                print *,"u_im,flag ",u_im,mofdata_depart(vofcomp+SDIM+1)
               enddo ! u_im=1..nmat
               stop
              endif
              ...
              do u_im=1,nmat
               ...
               dencomp_data=(u_im-1)*num_state_material+1
                ...
                ! old state variable.
               donate_density= &
                conserve(D_DECL(idonate_CELL,jdonate_CELL,kdonate_CELL), &
                         iden_base+dencomp_data) 
               massdepart=donate_density
               massdepart_mom=donate_density*added_weight(u_im)

               if (massdepart.le.zero) then
                print *,"density invalid in vfrac unsplit 2"
                print *,"icrse,jcrse,kcrse ",icrse,jcrse,kcrse
                print *,"idonate_CELL,jdonate_CELL,kdonate_CELL ", &
                  idonate_CELL,jdonate_CELL,kdonate_CELL
                print *,"u_im ",u_im
                print *,"donate_density ",donate_density
                print *,"multi_volume_depart(u_im) ",multi_volume_depart(u_im)
                print *,"massdepart ",massdepart
                stop
               endif

               if (massdepart_mom.le.zero) then
                print *,"density (for mom) invalid in vfrac unsplit 2"
                print *,"icrse,jcrse,kcrse ",icrse,jcrse,kcrse
                print *,"idonate,jdonate,kdonate ", &
                 idonate_CELL,jdonate_CELL,kdonate_CELL
                print *,"u_im ",u_im
               ...
               do veldir=1,SDIM
                mom2(veldir)=zero 
                donate_data= &
                 conserve(D_DECL(idonate_CELL,jdonate_CELL,kdonate_CELL),veldir)
                mom2(veldir)= &
                 multi_volume_depart(u_im)*massdepart_mom*donate_data
               enddo  ! veldir=1..sdim (velocity)

               massdepart=massdepart*multi_volume_depart(u_im)
               massdepart_mom=massdepart_mom*multi_volume_depart(u_im)
               ...
               istate=2
               do while (istate.le.num_state_material)
                statecomp_data=(u_im-1)*num_state_material+istate

                 ! conserve initialized in FORT_BUILD_CONSERVE.
                 ! Temperature and species variables are multiplied by 
                 ! dencore(u_im) in BUILD_CONSERVE.  (dencore(u_im) is the
                 ! value of density stored in the state variable)
                donate_data= &
                 conserve(D_DECL(idonate_CELL,jdonate_CELL,kdonate_CELL), &
                          iden_base+statecomp_data) 
                ...
                if (istate.eq.2) then
                 if (veldata(iden_base+statecomp_data).lt.zero) then
                  print *,"energy became negative "
                  print *,"u_im,comp2 ",u_im,iden_base+statecomp_data
                  print *,"current donated value ", &
                   veldata(iden_base+statecomp_data)
                  print *,"icrse,jcrse,kcrse ",icrse,jcrse,kcrse 
                  print *,"idonate_CELL,jdonate_CELL,kdonate_CELL ", &
               ...
               veldata(iLS_base+u_im)=veldata(iLS_base+u_im)+ &
                LS_voltotal_depart*donate_data

               vofcomp=(u_im-1)*ngeom_raw+1
               ! material volume from departure (donating) region
               veldata(imof_base+vofcomp)= &
                veldata(imof_base+vofcomp)+multi_volume_depart(u_im)
               ! material volume from target (accepting) region
               veldata(iFtarget_base+u_im)= &
                veldata(iFtarget_base+u_im)+multi_volume_target(u_im)

               ! material centroid from target (accepting) region
               do udir2=1,SDIM
                veldata(imof_base+vofcomp+udir2)= &
                 veldata(imof_base+vofcomp+udir2)+ &
                 multi_volume_target(u_im)*multi_cen_target(udir2,u_im)
               enddo 
              ...
              enddo ! u_im=1,..,nmat (state variables, geometry, velocity)
          ...
          do u_im=1,nmat
           vofcomp=(u_im-1)*ngeom_raw+1 
           volmat_target(u_im)=veldata(iFtarget_base+u_im)
           volmat_depart(u_im)=veldata(imof_base+vofcomp)

           ! fluid materials tessellate the domain.
           if (is_rigid(nmat,u_im).eq.0) then
            voltotal_target=voltotal_target+volmat_target(u_im)
            voltotal_depart=voltotal_depart+volmat_depart(u_im)
           else if (is_rigid(nmat,u_im).eq.1) then
            ! do nothing
           else
            print *,"is_rigid invalid"
          ...
          do u_im=1,nmat
           ...
           newvfrac(u_im)=volmat_target(u_im)/voltotal_target

           do udir2=1,SDIM
            if (newvfrac(u_im).gt.VOFTOL) then
             newcen(udir2,u_im)= &
              veldata(imof_base+vofcomp+udir2)/ &
              volmat_target(u_im)- &
              cencell_accept(udir2)
            else
             newcen(udir2,u_im)=zero
            endif
           enddo ! udir2=1..sdim

          enddo  ! u_im (geometry)

          call consistent_materials(newvfrac,newcen,nmat)
          ...
          do u_im=1,nmat
           ...
           if (dencore(u_im).le.zero) then
            print *,"density must be positive vfrac_split 2"
            print *,"u_im,dencore(u_im) ",u_im,dencore(u_im)
            stop
           endif
           if (dencore(u_im).lt.density_floor(u_im)) then
            dencore(u_im)=density_floor(u_im)
           endif
           if (density_ceiling(u_im).gt.zero) then
            if (dencore(u_im).gt.density_ceiling(u_im)) then
             dencore(u_im)=density_ceiling(u_im)
            endif
           else
            print *,"density_ceiling(u_im) invalid"
            stop
           endif
           snew_hold(istate)=dencore(u_im)

          enddo ! u_im, updating density

          ! levelset function
          ! voltotal_depart=sum_{fluid mat} volmat_depart(u_im)
          ! iLS_base=nmat*sdim+nmat*num_state_material+nmat*ngeom_raw
          do u_im=1,nmat
           newLS(u_im)=veldata(iLS_base+u_im)/voltotal_depart
          enddo  ! u_im
          ...
          if (totalmass_depart.le.zero) then
           print *,"totalmass_depart bust totalmass_depart=",totalmass_depart
           do udir2=1,SDIM
            print *,"udir2,fablo,fabhi ",udir2,fablo(udir2),fabhi(udir2)
           enddo
           print *,"icrse,jcrse,kcrse ",icrse,jcrse,kcrse
           print *,"num_state_material=",num_state_material
           print *,"nmat,level,finest_level ", &
          ...
          do u_im=1,nmat
           ...
           if (ngeom_raw.eq.SDIM+1) then
            snew_hold(mofcomp+vofcomp)=newvfrac(u_im)
            do udir2=1,SDIM
             snew_hold(mofcomp+vofcomp+udir2)=newcen(udir2,u_im)
            enddo
           else
            print *,"ngeom_raw invalid in vfrac split"
            print *,"ngeom_raw= ",ngeom_raw
           ...
           if ( (volmat_depart(u_im).le. &
                 VOFTOL*voltotal_depart).or. &
                (newvfrac(u_im).le.VOFTOL).or. &
                (volmat_target(u_im).le. &
                 VOFTOL*voltotal_depart) ) then
           ...
           do while (istate.le.num_state_material)
            ...
            if (istate.eq.1) then
             ...
             if (no_material_flag.eq.1) then
              snew_hold(dencomp+tempcomp_data)=fort_tempconst(u_im)
             else if (no_material_flag.eq.0) then
              if (is_rigid(nmat,u_im).eq.1) then
               if (fort_material_type(u_im).ne.999) then
                print *,"fort_material_type(u_im).ne.999"
                stop
               endif
               ...
               if (ETcore.lt.fort_tempcutoff(u_im)) then
                ETcore=fort_tempcutoff(u_im)
               endif
               if (ETcore.gt.fort_tempcutoffmax(u_im)) then
                ETcore=fort_tempcutoffmax(u_im)
               endif
          ...
          do u_im=1,nmat
           if (is_rigid(nmat,u_im).eq.0) then
            do istate=1,num_state_material
             statecomp_data=dencomp+(u_im-1)*num_state_material+istate
             snew(D_DECL(icrse,jcrse,kcrse),statecomp_data)= &
              snew_hold(statecomp_data)
            enddo ! istate=1..num_state_material
            ...
            if (solidheat_flag.eq.0) then ! diffuse in solid
             tempcomp_data=dencomp+(u_im-1)*num_state_material+2
             snew(D_DECL(icrse,jcrse,kcrse),tempcomp_data)= &
              snew_hold(tempcomp_data)
            else if (solidheat_flag.eq.2) then ! neumann
             ! do nothing
          ...
          do u_im=1,nmat
           if (is_rigid(nmat,u_im).eq.0) then
            LSnew(D_DECL(icrse,jcrse,kcrse),u_im)=newLS(u_im)

            ! level set comes from Lagrangian representation.
           else if (is_rigid(nmat,u_im).eq.1) then
            ! do nothing
           else
            print *,"is_rigid(nmat,u_im) invalid"
            stop
           endif
          enddo ! u_im=1..nmat

         else if (Imaskunsplit.eq.0) then
          ! do nothing
         else
          print *,"Imaskunsplit invalid"
          stop
         endif
      ...
      return
      end subroutine FORT_VFRAC_UNSPLIT
...
!         | (r S_31)_r/r + (S_32)_t/r +           (S_33)_z |
      ...
      subroutine traverse_particlesNODE( &
       accum_PARM, &
       matrixfab, &
       DIMS(matrixfab), &
       ncomp_accumulate)
      ...
      do interior_ID=1,accum_PARM%Npart
       ...
       do while (idx(3).le.subhi(3))
        ...
        if (interior_ok.eq.1) then
         ...
         j=idx(2)
         k=idx(3)
         call gridstenND_level(xsten,i,j,k,accum_PARM%level,nhalf)
         tmp=0.0d0
         do dir=1,SDIM
          xc(dir)=xsten(0,dir)
          tmp=tmp+(xpart(dir)-xc(dir))**2
         ...
         do dir=1,SDIM
          matrixfab(D_DECL(i,j,k),ibase)= &  ! ATb_1
           matrixfab(D_DECL(i,j,k),ibase)+ &
           w_p*1.0d0*xpartfoot(dir)
          ibase=ibase+1
          matrixfab(D_DECL(i,j,k),ibase)= &  ! ATb_2
           matrixfab(D_DECL(i,j,k),ibase)+ &
           w_p*(xpart(1)-xc(1))*xpartfoot(dir)
          ibase=ibase+1
          matrixfab(D_DECL(i,j,k),ibase)=  &  ! ATb_3
           matrixfab(D_DECL(i,j,k),ibase)+ &
           w_p*(xpart(2)-xc(2))*xpartfoot(dir)
          ibase=ibase+1
          matrixfab(D_DECL(i,j,k),ibase)= &  ! ATb_4
           matrixfab(D_DECL(i,j,k),ibase)+ &
           w_p*(xpart(SDIM)-xc(SDIM))*xpartfoot(dir)
          ibase=ibase+1
         enddo ! dir=1..sdim
         if (ibase.eq. &
             accum_PARM%matrix_points+accum_PARM%RHS_points*SDIM+1) then
      ...
      return
      end subroutine traverse_particlesNODE

      subroutine fort_assimilate_x_foot_from_particles( &
        tid, &  ! thread id
        tilelo,tilehi, &  ! tile box dimensions
        fablo,fabhi, &    ! fortran array box dimensions containing the tile
        bfact, &          ! space order
      ...
      call traverse_particlesNODE(accum_PARM, &
         matrixfab, &
         DIMS(matrixfab), &
         ncomp_accumulate)
      ...
      do k=growlo(3),growhi(3)
       ...
       do dir=1,SDIM
        TNEWfab(D_DECL(i,j,k),dir)=xLS(1,dir)
       enddo
      ...
      enddo
        ...
        !     (i) phi^{RK_stage+1} =f(phi^{RK_stage=0...RK_stage})
        ! 
      end subroutine fort_assimilate_x_foot_from_particles
