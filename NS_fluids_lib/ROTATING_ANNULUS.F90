...
subroutine ROTATING_ANNULUS_CFL_HELPER(time,dir,uu,dx)
...
if (dir.eq.0) then
 ...
 print *,"dir invalid ROTATING_ANNULUS_CFL_HELPER"
 stop
endif
...
return
end subroutine ROTATING_ANNULUS_CFL_HELPER
...
subroutine ROTATING_ANNULUS_check_vel_rigid(x,t,vel,dir)
use probcommon_module
IMPLICIT NONE
...
if (probtype.eq.55) then
 if (vel.eq.0.0d0) then
  ...
  print *,"ROTATING_ANNULUS_check_vel_rigid: vel not expected"
...
return
end subroutine ROTATING_ANNULUS_check_vel_rigid
  ...
  if (probtype.eq.55) then
   ...
   if (axis_dir.eq.3) then
    ...
    LS(2)=-LS(1)
     ...
     ! this routine: ROTATING_ANNULUS_LS (initial angle=static angle)
...
!  x(2)=xsten(0,2)
!  x(SDIM)=xsten(0,SDIM)
subroutine ROTATING_ANNULUS_HEATSOURCE(im,VFRAC,time,x, &
     xsten,nhalf,temp, &
...
return
end subroutine ROTATING_ANNULUS_HEATSOURCE
 ...
 ! heat_dir=1,2,3
 ! heat_side=1,2
subroutine ROTATING_ANNULUS_EB_heat_source(time,dt,xsten,nhalf, &
...
return
end subroutine ROTATING_ANNULUS_EB_heat_source

  ! only called at faces with an adjoining solid cell and
  ! an adjoining fluid cell.
subroutine ROTATING_ANNULUS_microcell_heat_coeff(heatcoeff,dx,veldir)
use probcommon_module
IMPLICIT NONE
...
return
end subroutine ROTATING_ANNULUS_microcell_heat_coeff

subroutine ROTATING_ANNULUS_velfreestream(problen,local_buffer)
use probcommon_module
IMPLICIT NONE
...
return
end subroutine ROTATING_ANNULUS_velfreestream


subroutine ROTATING_ANNULUS_nucleation(nucleate_in,xsten,nhalf,make_seed)
...
return
end subroutine ROTATING_ANNULUS_nucleation

subroutine ROTATING_ANNULUS_ICE_SUBSTRATE_DISTANCE( &
                xtarget,dist)
use probcommon_module
use global_utility_module
IMPLICIT NONE
...
end subroutine ROTATING_ANNULUS_ICE_SUBSTRATE_DISTANCE
...
subroutine ROTATING_ANNULUS_SUMINT(GRID_DATA_IN,increment_out1, &
...
! ns.ncomp_sum_int_user=4
    ...
    print *,"im_solid invalid ROTATING_ANNULUS_SUMINT"
    stop
   endif
  else
   print *,"num_materials invalid ROTATING_ANNULUS_SUMINT"
   stop
  endif
...
end subroutine ROTATING_ANNULUS_SUMINT
