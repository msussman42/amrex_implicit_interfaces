 ...
 for (int im=0;im<NavierStokes::num_materials;im++) {
  ...
  if (NavierStokes::material_type[im]==999) {
   ...
   if ((NavierStokes::FSI_flag[im]==FSI_FLUID)||
       ...
       (NavierStokes::FSI_flag[im]==FSI_ICE_EULERIAN_ELASTIC)||
...
NavierStokes::read_params ()
{
    ...
    if ((coarsest_level_CISL>=0)&&
        (coarsest_level_CISL<=max_level_for_use)) {
     //do nothing
    } else
     amrex::Error("expecting 0<=coarsest_level_CISL<=max_level_for_use");
    ...
    if (heatvisc_coef>0.0) {
     ...
     amrex::Error("heatvisc_coef invalid");
    ...
    } else if (unscaled_min_curvature_radius>=1.0) {
     //do nothing
    } else
    ...
    if (ns_max_grid_size[0]>=local_min_max_grid_size) {
     ...
     std::cout << "consider making ns.coarsest_level_CISL>0 \n";
...
void NavierStokes::assimilate_state_data() {
 ...
 MultiFab& Smac_new_x = get_new_data(Umac_Type,project_slab_step+1);
...
void NavierStokes::regenerate_from_eulerian(Real cur_time) {
 ...
 if ((nparts>=1)&&(nparts<=num_materials)) {
  ...
  MultiFab& Solid_new = get_new_data(Solid_State_Type,project_slab_step+1);
...
void NavierStokes::copy_old_FSI_to_new_level() {
 ...
 MultiFab& S_old=get_new_data(State_Type,project_slab_step);
 MultiFab& S_new=get_new_data(State_Type,project_slab_step+1);
 MultiFab* vofmf=
   getState(1,STATECOMP_MOF,num_materials*ngeom_raw,prev_time_slab);
 MultiFab& LS_old=get_new_data(LS_Type,project_slab_step);
 MultiFab& LS_new=get_new_data(LS_Type,project_slab_step+1);
 MultiFab* lsmf=getStateDist(1,prev_time_slab,local_caller_string);  
 MultiFab& Solid_old=get_new_data(Solid_State_Type,project_slab_step);
 MultiFab& Solid_new=get_new_data(Solid_State_Type,project_slab_step+1);
...
void NavierStokes::ns_header_msg_level(
 ...
 if (read_from_CAD()==1) {
  ...
  if (FSI_operation==OP_FSI_INITIALIZE_NODES) { 
   ...
   if (ns_level0.new_data_FSI[project_slab_step+1].CTML_num_solids==0) {
    //do nothing
   } else if (ns_level0.new_data_FSI[project_slab_step+1].CTML_num_solids==
 	      CTML_FSI_numsolids) {
    if (ns_level0.new_data_FSI[project_slab_step+1].max_num_nodes[0]==
        CTML_max_num_nodes_list[0]) {
     FSI_input.copyFrom_FSI(ns_level0.new_data_FSI[project_slab_step+1]);
     FSI_output.copyFrom_FSI(ns_level0.new_data_FSI[project_slab_step+1]);
    } else
     amrex::Error("new_data_FSI[project_slab_step+1].max_num_nodes incorrect");
   } else
    amrex::Error("new_data_FSI[project_slab_step+1].CTML_num_solids incorrect");
   ...
   if (ns_level0.new_data_FSI[project_slab_step+1].CTML_num_solids==
       CTML_FSI_numsolids) {
    if (ns_level0.new_data_FSI[project_slab_step+1].max_num_nodes[0]==
        CTML_max_num_nodes_list[0]) {
     FSI_input.copyFrom_FSI(ns_level0.new_data_FSI[project_slab_step]);
     FSI_output.copyFrom_FSI(ns_level0.new_data_FSI[project_slab_step+1]);
    } else
     amrex::Error("new_data_FSI[project_slab_step+1].max_num_nodes incorrect");
   } else
   ...
   if (ns_level0.new_data_FSI[project_slab_step+1].CTML_num_solids==
    ...
    if ((ns_level0.new_data_FSI[project_slab_step+1].max_num_nodes[0]==
         CTML_max_num_nodes_list[0])&&
        (ns_level0.new_data_FSI[project_slab_step+1].max_num_nodes[1]==
         CTML_max_num_nodes_list[1])&&
        (ns_level0.new_data_FSI[project_slab_step+1].max_num_nodes[2]==
  ...
  if ((FSI_operation==OP_FSI_INITIALIZE_NODES)||  // initialize nodes
   ...
   if (FSI_operation==OP_FSI_INITIALIZE_NODES) { 
    ...
    ns_level0.new_data_FSI[project_slab_step+1].copyFrom_FSI(FSI_output); 
...
NavierStokes::initData () {
 ...
 for (int dir=0;dir<AMREX_SPACEDIM;dir++) {
  MultiFab& Smac_new = get_new_data(Umac_Type+dir,project_slab_step+1);
...
void NavierStokes::init_boundary(
 ...
 for (int k=0;k<nstate;k++) {
  ...
  if (k==Umac_Type) {
   ...
   MultiFab& S_new=get_new_data(k,project_slab_step+1);
   scomp[k]=ncomp_total;
   ncomp[k]=S_new.nComp();
   ncomp_total+=ncomp[k]; 
 ...
 for (int k=0;k<nstate;k++) {
  ...
  if (k==Umac_Type) {
   ...
   if (control_flag==NULL_CONTROL) {
    ...
    MultiFab& S_new_extra=get_new_data(k,ns_time_order+extra_comp+1);
    MultiFab::Copy(S_new,S_new_extra,0,0,ncomp[k],1);
   } else
...
void NavierStokes::LSA_save_state_data(int extra_comp,int control_flag_in) {
 ...
 if (control_flag_in==LSA_SAVE_CONTROL) {
  ...
  for (int dir=0;dir<AMREX_SPACEDIM;dir++) {
   ...
   MultiFab& umac_extra=get_new_data(Umac_Type+dir,ns_time_order+extra_comp+1);
   MultiFab::Copy(umac_extra,*temp_mac_velocity,0,0,1,0);
  ...
  if (project_slab_step==ns_time_order-1) {
   ...
   amrex::Error("expecting project_slab_step==ns_time_order-1");

  for (int dir=0;dir<AMREX_SPACEDIM;dir++) {
   MultiFab& umac_new=get_new_data(Umac_Type+dir,project_slab_step+1);
   MultiFab& umac_extra=get_new_data(Umac_Type+dir,ns_time_order+extra_comp+1);
   MultiFab::Copy(umac_new,umac_extra,0,0,1,0);
  }
 } else
  amrex::Error("control_flag_in invalid");
...
void NavierStokes::LSA_save_state_dataALL(int extra_comp,int control_flag_in) {
 ...
 for (int ilev=level;ilev<=finest_level;ilev++) {
  NavierStokes& ns_level=getLevel(ilev);
  ns_level.LSA_save_state_data(extra_comp,control_flag_in);
 } //ilev
...
#endif
{
 for (MFIter mfi(S_extra_comp,use_tiling); mfi.isValid(); ++mfi) {
  ...
  FArrayBox& S_evec=S_extra_comp[mfi]; 
  FArrayBox& LS_evec=LS_extra_comp[mfi]; 

  int tid_current=ns_thread();
  thread_class::tile_d_numPts[tid_current]+=tilegrid.d_numPts();

  int state_ncomp=S_evec.nComp();
  ...
  fort_default_evec( 
   ...
   &state_ncomp,
   S_evec.dataPtr(),
   ARLIM(S_evec.loVect()),ARLIM(S_evec.hiVect()),
   LS_evec.dataPtr(),
   ARLIM(LS_evec.loVect()),ARLIM(LS_evec.hiVect()),
   tilelo,tilehi,
   fablo,fabhi,
   &bfact,
   xlo,dx,
 ...
 if (1==1) {
  ...
  for (int i=0;i<num_materials;i++) {
   std::cout << "i = " << i << '\n';
   std::cout << "LS_extra_comp.norminf(i) = " << 
      LS_extra_comp.norminf(i) << '\n';
   std::cout << "LS_unperturb_extra_comp.norminf(i) = " << 
      LS_unperturb_extra_comp.norminf(i) << '\n';
  }
 }
...
void NavierStokes::LSA_eigenvector(
 int unperturb_extra_comp,int extra_comp) {
...
void NavierStokes::LSA_normalize_eigenvector(
 ...
 if (LS_cell_max.size()==num_materials) {
  ...
  amrex::Error("LS_cell_max.size()==num_materials failed");

 int ncomp_local=S_extra_comp.nComp();

 Vector<Real> scale_parm(ncomp_local);
 for (int dir=0;dir<ncomp_local;dir++) {
  scale_parm[dir]=0.0;
 }
 Vector<Real> LS_scale_parm(num_materials);
 for (int dir=0;dir<num_materials;dir++) {
 ...
 if (1==1) {
  ...
  for (int i=0;i<LS_scale_parm.size();i++) {
   std::cout << "i= " << i << " LS_scale_parm[i]= " << LS_scale_parm[i] << '\n';
  }
 }
...
NavierStokes::SDC_setup_step() {
 ...
 if ((slab_step>=0)&&(slab_step<ns_time_order)) {
  ...
  prev_time_slab=state[State_Type].slabTime(project_slab_step);
  cur_time_slab=state[State_Type].slabTime(project_slab_step+1);
...
NavierStokes::prepare_displacement() {
 ...
 for (int normdir=0;normdir<AMREX_SPACEDIM;normdir++) {
  ...
  // 1. multiply velocity by local_dt.  
...
NavierStokes::SEM_scalar_advection(int init_fluxes,int source_term,
 ...
 if (enable_spectral==1) {
  ...
  MultiFab& S_new_test=get_new_data(State_Type,project_slab_step+1);
...
NavierStokes::split_scalar_advection(int im_extension) { 
 ...
 MultiFab& Tensor_new=get_new_data(Tensor_Type_local,project_slab_step+1);
...
#endif
{
   for (MFIter mfi(S_new,use_tiling); mfi.isValid(); ++mfi) {
    ...
    FArrayBox& refinedennewfab=Refine_Density_new[mfi];
    ...
    fort_vfrac_split(
     ...
     refinedennewfab.dataPtr(),
     ARLIM(refinedennewfab.loVect()),
     ARLIM(refinedennewfab.hiVect()),
     LSdestfab.dataPtr(),
...
#endif
{
  ...
  amrex::Error("level invalid in split_scalar_advection");
 ...
 if (do_plot==1) {
  ...
  if (visual_output_raw_mac_Type==1) {
   ...
   for (int dir_mac=0;dir_mac<AMREX_SPACEDIM;dir_mac++) {
    MultiFab& mac_new_temp=get_new_data(Umac_Type+dir_mac,project_slab_step+1);
    writeSanityCheckData(
     "RawMacType",
     "RawMacType: vel",
     local_caller_string,
...
void NavierStokes::writeSanityCheckData(
 ...
 if (data_mf>=0) {
  ...
  if (state_type_mf>=0) {
   raw_data_lev0_mf=&get_new_data(state_type_mf,project_slab_step+1);
  } else {
 ...
 for (int ilev=tecplot_finest_level;ilev>=coarsest_level_CISL;ilev--) {
  ...
  if (data_mf>=0) {
   ...
   if (state_type_mf>=0) {
    raw_data_mf=&ns_level.get_new_data(state_type_mf,project_slab_step+1);
   } else {
    raw_data_mf=nullptr;
    amrex::Error("state_type_mf invalid");
   }
...
void NavierStokes::DumpProcNum() {
 ...
 bool use_tiling=ns_tiling;
 MultiFab& unew = get_new_data(State_Type,project_slab_step+1);
...
NavierStokes::avgDown(int stateidx,int startcomp,int numcomp,
 ...
 if (level==finest_level) {
  ...
  MultiFab& S_crse = get_new_data(stateidx,project_slab_step+1);
  MultiFab& S_fine = fine_lev.get_new_data(stateidx,project_slab_step+1);
...
void NavierStokes::MOFavgDown() {
 ...
 MultiFab& S_fine=fine_lev.get_new_data(State_Type,project_slab_step+1);
 MultiFab& S_crse = get_new_data(State_Type,project_slab_step+1);
...
void NavierStokes::avgDown_refine_density(int im_comp) {
 ...
 MultiFab& S_fine=fine_lev.get_new_data(Refine_Density_Type,project_slab_step+1);
 MultiFab& S_crse = get_new_data(Refine_Density_Type,project_slab_step+1);
...
void NavierStokes::avgDown_refine_tensor(int scomp) {

 std::string local_caller_string="avgDown_refine_tensor";
 ...
 MultiFab& S_fine=fine_lev.get_new_data(Tensor_Type,project_slab_step+1);
 MultiFab& S_crse = get_new_data(Tensor_Type,project_slab_step+1);
 ...
 if (grids!=S_crse.boxArray())
  amrex::Error("S_crse invalid avgDown_refine_tensor");
 if (fgrids!=S_fine.boxArray())
  amrex::Error("S_fine invalid");
 if (S_crse.nComp()!=S_fine.nComp())
  amrex::Error("nComp mismatch");
...
MultiFab* NavierStokes::getStateDIV_DATA(int ngrow,
 ...
 MultiFab& S_new=get_new_data(DIV_Type,project_slab_step+1);
 int ntotal=S_new.nComp();
 if (ntotal!=1)
  amrex::Error("ntotal invalid");
...
MultiFab* NavierStokes::getStateMAC(int ngrow,int dir,Real time) {
 ...
 MultiFab& S_new=get_new_data(Umac_Type+dir,project_slab_step+1);
 int ntotal=S_new.nComp();
