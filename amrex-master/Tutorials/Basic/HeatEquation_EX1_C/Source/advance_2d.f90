...
subroutine get_boundary_condition(x,y,tn,tnp1,probtype, &
 ...
 return
 end subroutine get_boundary_condition

subroutine get_source_term(x,y,tn,tnp1,probtype, &
  fsource)
 use amrex_fort_module, only : amrex_real
 IMPLICIT NONE

 real(amrex_real), intent(in) :: x,y,tn,tnp1
 real(amrex_real), intent(out) :: fsource
...
subroutine update_phi (lo, hi, phiold, polo, pohi, phinew, pnlo, pnhi, &
                       ...
                       dx, dt, &
                       problo,time_n,time_np1, &
                       probtype) bind(C, name="update_phi")
  ...
  integer lo(2), hi(2), polo(2), pohi(2)
  integer pnlo(2), pnhi(2), fxlo(2), fxhi(2)
  integer fylo(2), fyhi(2)
  ...
  real(amrex_real) :: fsource,x,y

  dtdx = dt/dx

  do j = lo(2), hi(2)
  do i = lo(1), hi(1)
     x=problo(1)+(i-lo(1)+0.5d0)*dx(1)
     y=problo(2)+(j-lo(2)+0.5d0)*dx(2)
     call get_source_term(x,y,time_n,time_np1, &
      probtype,fsource)
     phinew(i,j) = phiold(i,j) &
          + dtdx(1) * (fluxx(i+1,j  ) - fluxx(i,j)) &
          + dtdx(2) * (fluxy(i  ,j+1) - fluxy(i,j)) + dt*fsource
