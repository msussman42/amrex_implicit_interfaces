...
subroutine crossingTime(lo, hi, dx, &
 ...
 !do n = 1, 24 !crossing time iterations !should be no more than max width of narrow band !!fix this
  do m = 1, maskSize !iterate through mask
   ...
   elseif (mask(i+1,j,k) .EQ. 0  .OR.  mask(i+1,j,k) .EQ. 1 ) then
   ...
   elseif (mask(i,j+1,k) .EQ. 0  .OR.  mask(i,j+1,k) .EQ. 1 ) then
   ...
   elseif (mask(i,j,k+1) .EQ. 0  .OR.  mask(i,j,k+1) .EQ. 1 ) then
   ...
   if (n.LE.21) then !phi(i,j,k) .LT. 0.0 .AND.  !phi<0
    phi_new(m) = phi_old(i,j,k)-DOT_PRODUCT(-grad_phi/norm_grad_phi,grad_phi)*dtau !+norm_grad_phi*dtau
   ...
   elseif (n.GT.21) then !phi(i,j,k) .GT. 0.0 .AND.   !phi>0
    phi_new(m) = phi_old(i,j,k) +DOT_PRODUCT(-grad_phi/norm_grad_phi,grad_phi)*dtau !-norm_grad_phi*dtau
    !print *, "phi_old: ", phi_old(i,j,k), " phi_new: ", phi_new(m)
