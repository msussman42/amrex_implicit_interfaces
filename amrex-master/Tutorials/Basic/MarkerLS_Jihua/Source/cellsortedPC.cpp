...
//PC_Interface::
//PC_Interface(const Geometry            & a_geom,
//             const DistributionMapping & a_dmap,
//             const BoxArray            & a_ba)
//    : ParticleContainer<4,4,0,3> (a_geom, a_dmap, a_ba)
//{
//}
...
{
    BL_PROFILE("PC_Interface::InitParticles");
...
void
PC_Interface::UpdateCellVectors()
{
    BL_PROFILE("PC_Interface::UpdateCellVectors");
 ...
 //   if (not needs_update) return;  //!!put this back in, need to rebin as particles move out of cells instead of remapping

    ////std::cout << "interface cell map updated" << std::endl;
    // clear old data
...
#pragma omp parallel
    ...
    for (MyParIter pti(*this, lev); pti.isValid(); ++pti)
    {
        ...
        for(int pindex = 0; pindex < np; ++pindex) {
            ...
            p.idata(IntData_interface::sorted) = 1;
            p.idata(IntData_interface::i) = iv[0];
            p.idata(IntData_interface::j) = iv[1];
            p.idata(IntData_interface::k) = iv[2];
...
void
PC_Interface::UpdateFortranStructures()
{
    BL_PROFILE("PC_Interface::UpdateFortranStructures");
...
/*
void
PC_Interface::MoveParticles()
{
    BL_PROFILE("PC_Interface::MoveParticles()");
...
#pragma omp parallel
    ...
    for (MyParIter pti(*this, lev); pti.isValid(); ++pti)
    {
        ...
        {
	    ParticleType& p = particles[pindex];
	    if (p.idata(IntData_interface::sorted)) continue;
            const IntVect& iv = this->Index(p, lev);
            p.idata(IntData_interface::sorted) = 1;
            p.idata(IntData_interface::i) = iv[0];
            p.idata(IntData_interface::j) = iv[1];
...
void
PC_Interface::correctCellVectors(int old_index, int new_index, 
						int grid, const ParticleType& p)
{
    if (not p.idata(IntData_interface::sorted)) return;
    IntVect iv(p.idata(IntData_interface::i), p.idata(IntData_interface::j), p.idata(IntData_interface::k));
...
#pragma omp parallel reduction(+:num_unsorted)
    ...
    for (MyParIter pti(*this, lev); pti.isValid(); ++pti)
    {
        ...
        for(int pindex = 0; pindex < np; ++pindex) {
            ...
            if (p.idata(IntData_interface::sorted) == 0) {
                num_unsorted += 1;
            }
        }
    }
...
#pragma omp parallel reduction(+:num_wrong)
    ...
    for (MyParIter pti(*this, lev); pti.isValid(); ++pti)
    {
        ...
        for(int pindex = 0; pindex < np; ++pindex) {
            const ParticleType& p = particles[pindex];
            const IntVect& iv = this->Index(p, lev);
            if ((iv[0] != p.idata(IntData_interface::i)) or (iv[1] != p.idata(IntData_interface::j)) or (iv[2] != p.idata(IntData_interface::k))) {
                num_wrong += 1;
            }
        }
    }
...
void
PC_Interface::visitAllParticles()
{
    const int lev = 0;
